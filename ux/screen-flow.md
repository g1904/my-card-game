...
- **储物袋不进主菜单,它是轮回内的角色面板(已定案)。** 主菜单入口全是 **PlayerProfile 级**,而储物袋 `CharacterProfile.magicPack`(`List<CharacterItem>`)是 **CharacterProfile 级、随轮回清理**——入口因此挂在**角色状态条**上(点按「袋」图标)。
...
- **隐藏属性跨档时给一条定性叙事(已定案)。** 道心 / 煞气 / 寿元的**数值继续完全隐藏**,但**跨入一个离常态更远的档位时**在事件收口处给一条定性描述(「你于静室枯坐三日,心念澄明。」「你的指节泛起一层洗不去的暗红。」)。**只在跨档时触发**,不是每次结算都播——稀缺才有分量。玩家学到**方向与因果**,学不到精确数值,因而无法做电子表格式优化。规则与档位归 `systems/services/plot-manager.md`;它复用既有的 `eventEnd` 阶段,无新结构。Source: `handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
- **跨档叙事的呈现形态(已定案 · 08-12d · 答结原待答三问)。**
...
- **同步状态的两处呈现(已定案 · 08-09)。**
  - **常驻「离线 · 待同步 N」指示。** `sync-service` 的既定指示:UI 收到 `SyncStateChanged` 后单点查询 `PendingCount` 渲染。**它在战斗屏内同样必须可见**——这是「进入战斗前同步失败不额外提示」那条纪律成立的前提;若战斗屏隐藏了它,静默就变成了失联。见 `ux/combat-ux.md`。
    - **第三种取值(08-12):`UpgradeRequired == true` → `需更新 · 待同步 N`,点按打开更新引导半屏。** 此时**必须换掉「离线」二字**——「离线」隐含「正在重连、会自己好」,而 `Upgrade` 态本会话内永不恢复。三种取值的完整表、去重规则与更新引导形态见 `ux/error-and-blocking-ux.md`。
  ...
  Source: `handoffs/2026-08-09-sync-revision-cas-and-immediate-flush-nonblocking.md`;第三种取值 `handoffs/2026-08-12-error-copy-and-update-prompts.md`。
- **主菜单顶部的「建议更新」横幅(已定案 · 08-12)。** `X-Recommended-App-Version` 高于本机版本时,主菜单顶部(安全区内)出一条**可关闭横幅**;**每个 `recommendedVersion` 取值只提示一次**,**绝不在轮回内 / 战斗内呈现**。`UpgradeRequired == true` 时**不渲染本横幅**(高档吸收低档)。完整的三档去重规则见 `ux/error-and-blocking-ux.md`。Source: `handoffs/2026-08-12-error-copy-and-update-prompts.md`。
- **三种终局态共用一个阻塞屏(已定案 · 08-12)。** 需更新(强更)、被挤下线、存档读取失败三者形态同构,收敛为一个 `BlockingNoticeScreen` + 变体表(全屏 · 无返回 · 主按钮永不是「继续游玩」 · 底部诊断编号可长按复制)。**三个变体不等于三处硬阻塞**——阻塞点仍是既定两处。见 `ux/error-and-blocking-ux.md`。Source: 同上。
