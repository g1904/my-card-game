...
- **角色选择屏(主菜单的一个子步骤)。** 流程 = 主菜单 →「切换篇章」选中炼气 → **角色选择屏** → 确认即 `StartCycle`。**不新增主菜单入口**,上表五项不动;ch2 / ch3 续章与重试走角色继承,不经本屏。
...
- **寿元恒精确、明文常驻。** 寿元(lifeSpan)是角色唯一的资源命线,**余量常驻角色状态条并恒显示精确数值**;余量低于本章可用预算的 **10%** 时**转红字**——那是纯视觉强调,不是一条叙事通道。
  ...
  - **不做「大限将至」一类的预警文案。** 余量既已明文常驻且精确,预警型提示文案没有存在理由——玩家随时能自己读出还剩几步。**终态死亡屏对 `DefeatReason` 的呈现照常保留**(那是结果呈现,不是提示)。
- **EventOption 选择界面上有一条常驻的角色状态条。** 寿元余量的静态标注就落在这里,本次把它明确化为一条**角色状态条 + 一行进度 / 入口**:

  ```
  ┌─────────────────────────────────────┐
  │ [安全区]                             │
  │ 炼气七层 · ❤92 · ✦5/5 · 灵石 120     │  ← 角色状态条（❤ = 寿元精确余量，< 10% 转红字）
