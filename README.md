...
**当前状态：后端尚未开工，全库尚未进入可推导需求的阶段。** 下一步不是写实现，而是把 `contracts/` 的契约骨架立起来——见 `open-questions.md` 的「下一阶段」。
...
| `contracts/` | **本库的核心产出**：客户端 ↔ 后端协议契约的单一事实来源（信封、鉴权、profile 同步、内容 manifest）。 | 持续更新；**只保留最新契约**（兼容性靠版本化字段，不靠保留旧形态）。当前有两份：`envelope.md`（边界层：表达形式 · 序列化约定 · 信封 · 错误码台账 · 版本协商）与 `content-manifest.md`（内容分发）。 |
...
| `open-questions/` | 待答清单的**分片**：`01-contracts.md` … `06-platform-stack.md`（编号即优先级）、`update-log.md`（逐次更新摘要）。**只跟踪仍待答的问题**（无「已解决」区）。 | 持续更新。分片过长可再拆、过短可并回，同步更新索引导航表。 |
