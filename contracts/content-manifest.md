...
**`sig` 为 base64 编码的 IEEE P1363 `r‖s`（各 32 字节定长，共 64 字节），不是 ASN.1 DER。** `alg: ES256` 已隐含这一点，但隐含得不够显眼，而代价是一个只在真机集成期才暴露的失败：JOSE 的 `ES256` 定义为 P1363，客户端 .NET 的 `ECDsa.VerifyData(data, sig, HashAlgorithmName.SHA256)` 默认也期望 P1363，而多数密钥服务的签名 API 返回 DER。两侧各自都没错、合起来不通——签名生成毫无报错，客户端 100% 验签失败。发布流水线的编码归一步见 `operations/content-delivery-ops.md`。
