...
- 超出足以奠定 grimdark 基调之外的深度叙事/剧情内容。**叙事的承载形态本身已定**：世界观走碎片化 lore，只活在轮回内的事件与剧本正文里、读过即过，**不做设定集、不做 lore 图鉴、不做任何可回看的档案面**；内部留档归 `narrative/`（见 `narrative/_index.md`）。
- 玩法进度型的元进程解锁（靠通关 / 成就逐步解锁玩法内容）、每日/seeded-轮回分享。角色的「免费 / 付费」轨道不属此列，见下条。
- **后续角色系列 —— 确定项、后续版本引入。** 角色按系列成批扩张（免费的双灵根批 + 付费的进阶系列；整系列全免费或全付费）；**首批五角永久免费恒可用**；付费解锁角色是商业化第三支，**严格横向、不卖强度**。**次序：第一个付费系列（单灵根五角进阶批）在前、双灵根免费批在后**——这是唯一已定的排期口径，隔几个版本未定。形态权威见 `systems/monetization.md` 与 `systems/character-profile/_index.md`。
...
### 路线阶段（按系统层次 · `decisions/ADR-0006-development-phase-order.md`，Accepted）
**框架 → 内容 → 平衡与体验 → 社交及其他。**
...
- **设计密度远超实现进度是已知且接受的状态。** 当前 190 余份设计文档 · 0 行代码 · 0 份 FR，且库内大量「初值 / 待统计校准 / 待实测」标注意味着**相当一部分精算结论在第一次跑起来之前无法证伪**（`itemPowerRatio` 四档折价、战斗内法则的强度上沿都是实例）。**这不构成问题，无需处理**——**不中断设计去先落一个最小可跑的战斗回路**；「设计先行（自顶向下）」这条既定工程方法照常执行，悬空的数字先由形状锚点框住相对关系，具体取值留待内容扩充后的统计校准。
- 此为流程性方向决策，已固化为 `decisions/ADR-0006-development-phase-order.md`（Accepted）——该 ADR 同时覆盖上节的四个路线阶段与本节的工程方法，两节是同一条决策的两个视角。
