...
| `moderation.md` | **已建立**：昵称词表的不可变版本化发布与两档分级 · 第三方审核适配器的两阈值形态（服务商 × 环境维度，能力未启用即不定值）· 未过审昵称的存量扫描 T1/T2/T3 与处置阶梯（含「须改名」档豁免频次闸拦截、不豁免计数）· 风控事件流字段表 / **`kind` 取值（含注销申请与撤销两条审计事件）** / 阈值分档 / 全局熔断 / 自动化止于工单 · 数值初值 · 与合规域运维面的交叉点 |
