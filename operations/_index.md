...
本文件夹的文档已全部铺开，**各运维面的正文一律展开在下表的对应文档里**。本索引除文档表外，只保留两节**跨文档的交叉点**（购买与验票 · 版本兼容矩阵与错误码台账）——它们记的是某个运维面对其余运维面提出的要求与登记流程，落在任何单一文档里都会成为那份文档的越界内容。

## 文档

| 文档 | 覆盖 |
|---|---|
| `environments.md` | **已建立**：两套云上 + 本地环境实体 · 配置三层与旋钮清单 · 限流实现分层 · 区域与合规（境内主区、个人信息不出境、分表分权限）· 拓扑与副本（玩家读路径全走写入区、灾备只写能力要求）· 容量形状 · 会话 token 与内容签名两把密钥的保管与轮换对照 |
...
| `content-delivery-ops.md` | **已建立**：CDN 两类对象两种 TTL · 内容发布流水线七步与编码归一 · 发布侧内容校验闸 C1–C6 与留痕八字段 · flags 规则集的存储形态与变更通道 A1–A5（请求处理路径的服务内部形态见 `systems/content-delivery.md`） · 内容签名私钥的五条保管判据与 standby · `keyId` 轮换的三类触发 / 四阶段 / 覆盖率口径 · flags 读路径的拓扑自由度 · 数值初值 · 对存储 / 权限 / 密钥服务的栈中立能力要求 |
| `moderation.md` | **已建立**（人工那一端的形态在 `internal-tools.md`）：昵称词表的不可变版本化发布与两档分级 · 第三方审核适配器的两阈值形态（服务商 × 环境维度，能力未启用即不定值）· 未过审昵称的存量扫描 T1/T2/T3 与处置阶梯（含「须改名」档豁免频次闸拦截、不豁免计数）· 风控事件流字段表 / **`kind` 取值（含注销申请与撤销两条审计事件）** / 阈值分档 / 全局熔断 / 自动化止于工单 · 数值初值 · 与合规域运维面的交叉点 |
| `purchase-ops.md` | **已建立**：渠道验票凭据与轮换 · 三条退款对账通道 · 收据幂等记录的选型判据 / 分区 / 部分索引 / TTL 禁用断言 · 冷存归档的触发条件与三层形态（哨兵常驻 · 同库年度归档表 · 对象存储只作备份）· 两条对账信号与阈值 N 的校准口径 |
| `compliance-ops.md` | **已建立**：可信时钟（基准 = 事务库时钟 · 只 slew 不后跳 · 两处例外）· 时区与时段规则集的不可变版本化发布 · 法定节假日日历两类条目与判定顺序 · 三个失败面的降级语义 · `complianceTicket` 的存储与一次性消费（条件更新 + 60 秒回放做成寿命的延长）· 注销冷静期的 `SKIP LOCKED` 周期扫描与删除清单 · 数据导出的私有桶 / 现签短寿命链接 / 两道保留期 · 周期任务与六个旋钮的校准信号 |
| `internal-tools.md` | **已建立**：内部人员身份 `operator_id`（与 `account` 完全分离的四条理由 · 只停用不删行 · 全库四处「操作者」取值域的唯一落点）· 认证形态（一枚长期内部凭据、不建会话 · 企业 IdP 留位不启用且写死触发条件 · 第五类托管条目）· `/internal/` 接入面**不属于契约面**的三条判据与六端点最小集（明确不提供任意查询面）· 界面档次 CLI 与「人类身份对业务表恒只有 `SELECT`」· 复核台 `claimed_by` 的机械形态与判定落地 · `ops_ticket` 工单台与 `baseAccountStatus` 前置条件 · `operator_audit` · 可见字段范围 · 上线时点 · 服务端保证 I1–I10 · 数值初值 |
| `external-providers.md` | **已建立**：四类外接原子能力（短信 / 邮件 / 实名核验 / 第三方昵称审核）的适配接口共有形状 A1–A6 与四个签名 · 硬超时初值 · 三张归一映射表（按调用点所在域取 · `auth.challenge_expired` 永不来自服务商 · 实名拒绝归 `compliance.verification_failed`）· 逐能力的供应商数与灾备判据 · 短信降级 / 回切触发条件与禁并发双发 / 禁双验 · 选型判据 H1–H4 / S1–S4 · 凭据托管（能力 × 供应商 × 环境，独立审计线）· 可观测性增量与余额低水位告警 · 微信开放平台资质的前置链与过闸断言 |
...
- **各类钥匙不共用托管配置**：内容签名 ES256 私钥 · 会话 token 签名密钥 · **渠道验票凭据** · 外接能力凭据（`external-providers.md`）。各自的轮换节奏互不相同（跨客户端发版周期 / 只在服务端 / 由渠道方日程驱动 / 由各服务商驱动），共用一份托管配置会让最慢的那条绑架其余。
- **收据幂等表的 TTL 禁用断言见 `purchase-ops.md` §3c**（本索引不复述其形态与理由）。它与 O6「历史规则集永久保留」是同形取舍。
...
- **台账 ⇔ spec 枚举的一致性是一条机检断言**（`contracts/_index.md`「契约变更的完成判据」断言②）：台账每个 `code` 与 spec 错误码枚举**双向一一对应**。这条之所以值得机检，是因为它的漂移形态是**静默**的——客户端对未知 `code` 有兜底，漏登记不会报错，只会让某条错误一直走降级路径，线上表现为「处置分支不对」而非「报错」。承载为**设计库分支上的一条独立 workflow**（与镜像构建互不依赖，见 `deployment.md`）；**workflow 尚未落地，在此之前以人工清单执行**。
