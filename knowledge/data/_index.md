...
| `locationMap`（地域图） | `LocationMapData` | `game-progression.md`（**单份全局邻接表资源**，不由各 location 各持边） |
...
8. **可调数值存导出字段 / `BalanceData`**，绝不硬编码在系统逻辑里；**不散落 `ResourceLoader.Load`**，一切内容经 ContentRegistry。
9. **敌人与玩家共用 `CardData` 体系但不共用卡池**：`Pool` 是必填、无默认值，漏填即坏数据；**卡组规模两侧皆不设硬限**（代价由疲劳承接）。→ `systems/enemies/_index.md`
10. **敌人池归属的唯一权威是 `EnemyData.PoolScope`**（`LocationData` 不持敌人清单）；地域 / arc 专属条目是**叠加而非替代**——通用敌人恒可在任何地域出现。→ `systems/enemies/common-properties.md`
11. **本作不存在多敌人场景**——敌人实例单数，嵌在 `EventOption.Encounter` 内，不要预留 `List<EnemyInstance>`。→ `systems/adventure-event/combat/_index.md`
