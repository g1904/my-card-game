...
- **泄漏：** 见纪律 3。**调试期泄漏检查已定案（08-09e）**：`#if DEBUG` 订阅审计，**定位信息取 `d.Method` 不取 `d.Target`**（泄漏时实例已释放，反射元数据不依赖存活）——因此**订阅无需登记来源、`+=` 惯用形态原样保留**；**时机 = 切屏完成后由 game-progression 调一次**（零热路径成本），已否决「每次 `Emit` 顺带检查」（`GetInvocationList()` 分配数组、撞热路径不分配）；`_ExitTree` 只打计数摘要不做判定；**autoload 服务的订阅豁免**。→ `systems/architecture.md`。
