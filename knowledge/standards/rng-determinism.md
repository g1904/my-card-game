...
`.claude/rules/state-save-rules.md`（RNG 章节）的配套。**权威两份：`game-design-documents/systems/common-properties.md`**（带种子 RNG 通则、子流派生式、账号级随机的三参数派生）与 **`game-design-documents/systems/services/life-cycle-service.md`**（SeedManager、`Stream(RngStream)` 签名、`rng` schema 与字段类型）。**子流清单、派生式、schema、字段类型去那两份看，此处不复制。**
...
5. **战斗内随机直接用 `combat` 子流，不在其上再派生一层**——防 re-roll 已由决策点存档 + `State` 持久化封住；**篇章重试与续章各换一套全新随机流**（`RetryChapter` / `StartCycle` 各生成新 `cycleSeed` 与四条子流）。→ `systems/services/life-cycle-service.md`
