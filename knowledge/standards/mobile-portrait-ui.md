...
- **轮回内主导航是月圆之夜式的 eventOptions 横向滑动菜单，不是传统地图屏**——它是要打磨到位的那一个手势面。每项需显示 `selectCost` / `skipCost` / 是否 `ifMandatory`（强制项封死跳过通道）。权威见 `ux/screen-flow.md`。
- 按下时给予视觉/触觉反馈；让拖拽的可供性明显（抬起/放大被拖拽的卡牌）。
- **寿元告警的呈现形态已定：低于 10% 时由隐藏转为在屏显示，表达为「标红的数值倒数」**（red count-down numeral），而非常驻进度条。倒数的递减节奏、常驻哪些屏幕、是否伴随音效 / 震动仍待定 → `ux/screen-flow.md`、`ux/combat-ux.md`。
