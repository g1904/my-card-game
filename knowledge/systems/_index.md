...
| ├ 战斗 | `adventure-event/combat/` | TODO | 回合结构、敌人 AI、胜负结算。**`combatTier` 三档**（Practice / Standard / Finale）共用同一套代码，差异只在遭遇参数。 |
...
- **拆分轴 = 生命周期层 + 行为边界，不是数据类型**：**只有 Combat 真有状态机**，其余四类事件共享同一形状、差异在数据而非代码 ⇒ 落地为**两个** `IEventResolver` 实现，不是五个。→ `systems/services/_index.md`
