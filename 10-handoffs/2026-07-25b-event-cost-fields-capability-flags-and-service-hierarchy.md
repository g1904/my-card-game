...
- status: distilled
- distilled-to: terminology.md, 20-systems/adventure-event/common-properties.md, 20-systems/player-profile/player-power/common-properties.md, 20-systems/player-profile/player-power/_index.md, 20-systems/services/future-event-service.md, 20-systems/services/adventure-plot-service.md, 20-systems/services/life-cycle-service.md, 20-systems/architecture.md, 20-systems/common-properties.md, 20-systems/balance.md, open-questions.md, 10-handoffs/_index.md
...
- **`eventType` 是既有分类法的字段化落地**，与 `terminology.md` 九类枚举、`50-decisions/ADR-0002` 一致；Mystery 作为元类型遮罩一个固定事件的语义不变（被遮罩事件的真实 `eventType` 在揭示前不可见）。
...
- **`eventStart` / `eventEnd` 与 life-cycle-service `AdvanceEvent` 的职责边界。** 谁写 CharacterProfile、谁扣成本、谁推拉隐藏属性、谁触发 eventOptions 重算？签名与返回（结算结果对象？）未定。→ `20-systems/adventure-event/common-properties.md`、`20-systems/services/life-cycle-service.md`。
- **ViewModel 层是否需要单独一份文档？**（第 5 节方案已采纳并在 `20-systems/architecture.md` 显式化。）单列文档还是归 `40-ux/` 待定。
- **capability flag / modifier 的落地细节？**（第 6 节方案已采纳。）仍需定：flag 的枚举 / 命名空间、聚合面的宿主服务（见缺口 1）、`status` 与「拥有 / 失去」两态的存档表达、冲突 / 叠加规则（两个 power 授予同一 flag、modifier 的运算顺序）。→ `20-systems/player-profile/player-power/common-properties.md`。
- **PlayerProfile 侧待完善（用户已确认为待办）。** 账号级的服务与结构需一次专门设计：是否新增 **player-profile-service**、其 API 面（PlayerPower 获取 / 失去与 `status` 持久化、PlayerItem 次数扣减、成就进度与奖励发放、登录 / 云端同步）、capability 聚合面的宿主归属。→ `20-systems/player-profile/`、`20-systems/architecture.md`。
- **元婴 +500 的用途。** 既然无玩法影响，最终寿元值是否被终局结算 / 成就 / 排行读取？若否，是否值得保留该字段更新？→ `20-systems/balance.md`、`40-ux/screen-flow.md`。
- **架构缺口 2–7 各自的归属与优先级**（见第 7 节；缺口 1 = PlayerProfile 侧已确认为待办，缺口 8 已由 ViewModel 定案闭合）：是否新增 combat-service / save-sync-service？本地 `.tres` 与云端下发内容的分界如何划？life-cycle-service ↔ future-event-service 的编排顶点是谁？→ `20-systems/architecture.md`。

## Notes / triage

- 第 5 / 6 节是**应用户要求给出的方案**，**已由用户确认采纳**，在主题文档中记为已定案并登记为 **ADR 候选**（展示层三层切分、capability flag + modifier pipeline、服务从属）。第 7 节的体检结论作为「闭环缺口」小节落入 `20-systems/architecture.md`。
- 第 1 节解掉了 `open-questions.md` 中「元婴阶段是否再加预算」；第 4 节修正了 `20-systems/architecture.md` 与各服务文档中「三个服务并列」的表述。
