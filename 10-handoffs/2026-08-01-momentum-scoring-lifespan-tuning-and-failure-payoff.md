...
- date: 2026-08-01
- topic: 20-systems/scoring, 20-systems/character-profile/life + mana, 20-systems/adventure-event/combat, 20-systems/services/（combat / future-event / life-cycle / plot-manager）, 20-systems/balance, 20-systems/game-progression, 20-systems/player-profile/（enemy-codex / player-power）, 40-ux/（combat-ux / screen-flow）, 00-vision/（scope / references）, terminology
- status: distilled
- distilled-to: terminology.md, 20-systems/scoring.md, 20-systems/character-profile/（life.md, mana.md）, 20-systems/adventure-event/（common-properties.md, combat/**, practice/**）, 20-systems/services/（combat-service.md, future-event-service.md, life-cycle-service.md, plot-manager.md）, 20-systems/balance.md, 20-systems/game-progression.md, 20-systems/architecture.md, 20-systems/_index.md, 20-systems/player-profile/（enemy-codex/**, player-power/_index.md）, 40-ux/combat-ux.md, 40-ux/screen-flow.md, 00-vision/scope.md, 00-vision/references.md, open-questions.md, answer-logs/log-0801.md
...
`20-systems/adventure-event/common-properties.md` 现载「代码形态 = `ProfileChangeSpec`，`ChangeElement.BaseValue` **带符号**：负 = 消耗，正 = 产出」——**该约定不变**。`lifeSpanCost` 是**内容侧作者标注的正数量值（magnitude）**，由 **future-event-service 在物化组装 `selectCost` / `skipCost` 时取负**填入 `ChangeElement.BaseValue`。即：**内容作者写正数，spec 里仍是负数**，二者互不推翻。
...
- **`20-systems/scoring.md` 的答案就是道念。** 该文档此前整份为空——计分模型不再悬空：**计分 = 道念（momentum），且道念即战斗胜负判据**，不是独立于战斗之外的另一层。
...
- **combat-service 的胜负判定、`CombatSnapshot` / `PlayResult` 字段需含道念；`40-ux/combat-ux.md` 的战斗屏幕以道念为主视觉**（双方道念对比），life 退居次要。
...
- **危险度标注：Combat / Practice / Finale 不做模糊的危险度档位**（「同阶 / 略高 / 越阶 / 无从揣度」**否决**），**而是在 eventOptions 上精确标注敌人的等级**。
  - 推论：`40-ux/combat-ux.md` 的待决项「等级差本身是否对玩家可见」由此答定 —— **敌人等级精确可见**；玩家可自行与自身等级比对，从而理解**意图为何被遮蔽**（信息遮蔽有了可解释的因）。
...
- **各篇章 `lifeSpanCost` 的具体分档表。** 哪些事件类型多耗、单次幅度分别多少——需以目标时长 15–30 / 15–30 / 20–40 分钟**反推**。→ `20-systems/balance.md`、`20-systems/adventure-event/`。
- **道念模型的剩余机制。** ① 战斗的**终止条件**（回合上限？道念阈值？敌人卡组耗尽？）；② **道念的产出途径**（哪些卡 / 行为产道念）；③ **道念差 → life 损失的换算公式**。连锁改写本身已定案，不在此列。→ `20-systems/scoring.md`、`20-systems/services/combat-service.md`。
- **隐藏属性的档位划分。** 分几档、阈值在哪——**定性反馈的触发依赖它**；寿元的 30% / 10% 两段已给，道心 / 煞气的档位未给。→ `20-systems/services/plot-manager.md`。
- **道统残卷概率的累积规则与上限。** 累积粒度、上限、与 seed 公平性的关系。→ `20-systems/player-profile/player-power/`。
- **等级产出的频次与分布。** 「等级成长 = event reward」已定；但一章内需要多少个「升级型产出」才能把炼气从 1 爬到 13、它们如何分布在事件池中，未定——它与寿元预算的花法互相约束。→ `20-systems/balance.md`、`20-systems/game-progression.md`。
- **敌人等级精确标注的承载字段。** 「eventOptions 上精确标注敌人等级」意味着 `EventOption` 需在物化时携带敌人等级（或其来源）——它是物化字段清单里新的一项，且依赖尚未定的「敌人等级的来源」。→ `20-systems/services/future-event-service.md`。
- **评审问题 6 未裁决：无俯瞰地图 + 无预告 = 缺中长期规划感。** 进程是逐批择一的线性推进，玩家看不到前方，也无「还有几步到 Finale」的预告——中长期规划感从何而来，本次未讨论。→ `20-systems/game-progression.md`、`40-ux/`。
