...
#### 复制粘贴 / 卫生
- 重复代码块、从模板复制而未修改的名字、残留的 TODO/FIXME、不匹配的 `[System-Method]` 日志标签。

### 3. 报告 ⏸️
按严重程度分组发现：
- 🔴 **Bug**（运行期/编译失败）：缺 `using`/`partial`、未检查的 null 解引用、错误的信号签名、未播种的玩法 RNG、非原子存档、悬空的数据 id。
- 🟡 **Warning**：缺日志、命名不一致、可疑的 null 处理、逐帧分配。
- 🔵 **Info**：大 diff、值得手动查看的复杂逻辑、复用机会。
