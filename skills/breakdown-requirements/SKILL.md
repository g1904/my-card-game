...
description: 把一份 derive-requirements 产出的 FR 草稿拆成一个文件夹，内含若干更小的、可执行的子需求，每个都小到能被 /blueprint 一次吃下。闭合设计→代码链路的最后一环。
argument-hint: <FR-<id> | 空（列出可拆解的 FR）>
...
- 一个**薄纵切片**：能在 Godot 编辑器里**跑起来并观察到**（依据 `environment-rules.md`——验证靠游玩，不靠 CLI 测试）；
- 自成一个**可提交的增量**：做完它，项目仍可运行。

**切分启发式（按优先级）**
1. **按可观察行为切**，不按代码层切。「玩家能看到 X」优于「建好 X 层」。
