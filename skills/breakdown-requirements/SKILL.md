...
**粒度判据 —— 逐条比对 `<LIB>/requirements/_index.md` 的「拆解粒度判据」（软上界 / 软下界 / 纯数据资源型子需求的三条闸）**；超界或触发下界时不硬拒，但须在拆解 `_index.md` 写一行理由，否则再切 / 再并。阈值表本身不在此复述——两份表会各自漂移。

**切分启发式（按优先级）**
1. **按可观察行为切**，不按代码层切。「玩家能看到 X」/「客户端发 X 会收到 Y」优于「建好 X 层」。
2. **纯横向层能否独立成子需求，按上述判据里的三条闸判**（不过闸 ⇒ 并入消费它的子需求），别凭举例的感觉判。
...
- **子需求 `status` 继承父 FR 的签核状态**（父已是 `broken-down` 的增量拆解读其 `signed-off-as`）；**唯一例外：本子需求 `## Open questions` 非空 ⇒ 一律 `draft`**，漏掉它会产出台账写着 `ready`、进 `/blueprint` 却被当未签核拦下的子需求。规则见 `<LIB>/requirements/_index.md` 的「两层结构」。
...
### 7. 已知边界（照实说，不要自己扩权）
- **签核语义：** 子需求的签核状态按 `<LIB>/requirements/_index.md`「两层结构」的规则产出（继承 + Open-questions 例外闸），不要按旧习惯一律产出 `ready`。
- **切分结构是评审面，不是状态词：** 切成几片、边界画在哪、`depends-on` 怎么串，父 FR 里没有这些信息 ⇒ 在报告中点名请用户过一眼拆解 `_index.md`（一览 + 覆盖映射表 + 构建顺序）。不为此新增状态词。
...
### 请过一眼（拆解结构评审面）
- 切分与 depends-on 链见 requirements/FR-<id>/_index.md
- 超界 / 触发下界并写了理由的子需求：<id + 一句理由>｜无
