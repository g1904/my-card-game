...
- **空（无参数）** → 读 `open-questions.md` 的分片导航表并逐份读取分片，按主题列出待答条目，并**标注哪些适合本技能**（可由通行做法 / 既有约定推演）vs **哪些必须由用户取向决定**（纯玩法手感、美术基调、商业化取舍——这类若聚成一个重大主题，指向姊妹技能 `/design-direction-interview`，它以多轮取向 interview 产出定案式的 `inbox/design-draft-<slug>.md`）。询问处理哪一个。
...
**分类纪律（收紧 `[取向选择]`）：** `[取向选择]` 只保留给**真取向**——无客观最优、答案取决于用户的玩法 / 产品取向的选择。凡存在行业通行做法、工程常识默认、或可由既有约束推演出明显合理答案的子项（错误处理形态、命名 / 结构常规、边界情形的标准处理、有公认默认值的技术参数），一律归 `[通行做法]` / `[既有推演]` 直接给出建议方案，其余选项一句话写进「备选方案（已考虑并否决）」即可。判据：一个有经验的从业者不会为它开会 → 不是取向。`## 仍需用户决定` 以 **0–2 条**为目标；明显超出时回头检查是否把有标准答案的问题误判成了取向。
...
### 下一步（评审后直接复制运行）
/analyze-new-ideas <详细提示词，见下方构造规则>
```

**「下一步」提示词的构造规则（强制）——不要只给一行裸命令**，要产出一段用户评审完草稿后**可直接复制运行**的完整提示词，包含：

1. **完整草稿路径**：`<LIB>/inbox/solution-draft-<slug>.md`；跨库时**两份 counterpart 路径都列上**，并注明「两份须成对提炼、两侧互相回链」。
2. **评审状态前提**：草稿尚待评审时写「评审通过后运行；若有修改以修改后的草稿为准」；若草稿有 `## 仍需用户决定` 项，点明「<K> 项取向裁决后运行（裁决可直接写进草稿该条目下）」。
3. **写库授权**：单库写「作用于 <LIB>」；跨库写「授权本次对 game-design-documents/ 与 backend-design-documents/ 两库写入」。
4. **顺带事项**（有则写）：推演过程中发现的、答案已定、纯机械的落笔项（如某台账行待回链、某承接项待登记），一句话点明「顺带落笔：<事项>（纯机械，答案已定）」——让下一步一次收口，不留尾巴。

示例形态（跨库场景）：
```
/analyze-new-ideas game-design-documents/inbox/solution-draft-<slug>.md backend-design-documents/inbox/solution-draft-<slug>.md — 两份为 counterpart 草稿，须成对提炼、两侧互相回链；<K> 项取向已在评审中裁决（以草稿内「→ 已裁决」为准，视同用户拍板）。授权本次对两库写入。
```
