...
- 若其权威主题文档现已在 `## 决策` / `## 意图`（或 `50-decisions/ADR-*`）给出定论 → 从所在分片移除，并在**本次的 answer log**（第 5b 步）记一行，附归档去向（`文档路径`）与一句结论。**不在任何分片里保留「已解决」区。**
- 若仍无定论 → 保留。
...
- **索引 `open-questions.md`**：更新「最近更新：<今天日期>」一句、以及分片导航表（若新增 / 合并了分片）。结构：说明 → `## 分片导航` → `## 当前焦点` → `## derive 就绪度`（**原样保留，不得改动**）→ `## 下一阶段`。索引里**不放问题条目**。
- **`open-questions/update-log.md`**：在**顶部**追加本次的更新摘要（答结了什么、推翻了什么、新增待答落在哪个分片、对应 answer log 文件名）。
