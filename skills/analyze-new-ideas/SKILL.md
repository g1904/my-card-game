...
| 🟠 **含糊** | 陈述存在两种以上合理解读，不同解读会导出**不同的文档内容 / 机制 / 数值**，**且无法由通行做法 / 工程常识 / 既有设计推演选出明显合理的默认解读**（真正取决于用户取向） | **必须 interview** |
| 🔵 **可推演** | 意图明确，与既有设计一致或能由既有设计逻辑必然推出；**或虽有多种解读，但行业通行做法 / 工程常识给出明显合理的默认、其余解读无可见优势** | 不必 interview；按推演 / 默认直接落笔，在报告中列出依据 |

**级别判定纪律（必问过滤）：** interview 只承载两类问题——**真冲突**（🔴：与用户既有定案相抵、或输入自相矛盾）与**真取向**（🟠：无客观最优、答案取决于用户的玩法 / 产品取向）。凡属**基础工程常识、行业标准实践、或由既有决策逻辑可推演**的问题（错误处理形态、命名 / 结构常规、边界与退化情形的标准处理、有公认默认值的技术参数），**不要问**——按 🔵 直接采纳标准答案。判据：一个有经验的从业者不会为它开会 → 不问。两个方向的拿不准分开处理：拿不准**是不是取向问题** → 往 🟠 判（问）；只是拿不准**具体形态、但存在通行做法** → 往 🔵 判（采纳默认）。每多问一轮都是一次打断——问题的数量本身就是成本，把它花在真取向与真冲突上。

**不要**用"先按最合理的一种解读写下去，再在 Open questions 里备注"来绕过**真冲突与真取向**的 🔴 / 🟠——标准默认的采纳是 🔵 的正常路径，不算绕过。Open questions 只承载**用户答完之后仍然开放**的远期未知，不承载"我本可以问但没问"的含糊。

### 4. Interview 澄清阶段 ⏸️（强制门）

**第 3 步存在任何 🔴 或 🟠 → 停在这里，一个文件都不写**（handoff 也不写）。参照 `/blueprint` 的澄清检查点：一次性把问题摆齐，不要挤牙膏式地问一轮做一点。**出题前先跑一遍必问过滤（3c）**：清单里只该剩真冲突与真取向；被过滤掉的项直接按标准默认落笔并在报告「标准默认」区列出，不占 interview。
...
5. **用户明确授权「按你的判断来 / 不要问」** → 按推荐项**直接定案**：视同用户拍板，正常写入设计文档，在 handoff 的 Clarifications 记一行 `<问题> → <采纳的推荐>（用户授权按推荐定案）`。不加标记、不留待决项。
...
- **Clarifications（interview 产物）：** 第 4 步问出来的每一项，记为 `<问题> → <用户裁决>`，并注明它推翻 / 细化了原始输入的哪一句。这是这份 handoff 相对原始记录**多出来的意图**，必须可见。本次自动采纳的标准默认项也记在这里（`<问题> → <采纳的默认 + 依据>`）——它们**不进** Open questions（已按标准答案定下，用户不认可时再推翻）。
...
- **矛盾在第 3–4 步就该问掉。** 走到这一步仍留着的矛盾，只有两种合法来源：用户已明确裁定或授权按推荐定案（按定案写入，并在 handoff 记一句依据）；或矛盾**取决于一个用户当下也答不出的远期未知**（留在 Open questions，并写明已问过、待后续拍板）。除此之外**不允许**把矛盾"记进 Open questions"当作绕过 interview 的出口。
...
- 向用户汇报：
  - 目标库（判定依据）。
  - **校验与 interview 结果**：问了哪几项、用户如何裁决（含授权按推荐定案的项）、哪些 🔵 是你自行推演的（附依据）。
