...
| 入口 | 上游 | 下游 |
|-------|----------|-----------|
| UI/屏幕场景 | 什么打开/承载它 | 系统调用 → RunState → EventBus |
| 系统/管理器 | 调用者（UI、其他系统、EventBus 订阅者） | RunState 变更、DataRegistry 读取、save 写入 |
| Autoload（RunState/EventBus/DataRegistry/SaveManager） | 所有发出者/消费者 | 它所拥有的数据/服务 |
...
- **类型一致性**：input → 系统 → RunState → 数据 → save，参数/返回类型对齐。信号载荷为 Variant 简单类型。
...
- **状态/RNG/存档**（`state-save-rules.md`）：seeded 子流随机性、由 RunState 拥有的变更、干净的 run 拆解、原子且带版本的存档、基于 id 的 save 引用。
- **UI/输入**（`ui-input-rules.md`）：竖屏容器/锚点、触摸目标、无仅悬停。
- **事件/信号设计**（`signal-eventbus.md`）：跨系统经 EventBus 解耦；无重入循环；在顺序重要处不假设顺序（如 relic 触发优先级）。
- **业务逻辑**（若上下文允许）：分支覆盖（空牌堆、run 中途恢复、boss vs 普通）、状态机合理性、无效果的重复施加。
...
- 🟡 **Warning**：缺日志、命名不一致、null 安全存疑、run 状态在 run 间的可疑传递、逐帧分配。
