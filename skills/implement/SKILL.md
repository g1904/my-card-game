...
### 4. 验证说明
**不要**把 CLI 编译当作事实来源。告诉用户通过在 Godot 编辑器中打开项目并按下 Play 来验证（编辑器会用正确的引用驱动 .NET 构建）。对 `.csproj` 运行 `dotnet build` 能捕获 C# 语法错误，但不具权威性。除非要求，否则不写单元测试。
...
在变更摘要中列出更新过的笔记。设计意图本身（`game-design-documents/`）仍归用户所有——不要反向改设计文档。若怀疑知识与代码已大面积脱节，建议运行 `/sync-knowledge` 做整体对账。
