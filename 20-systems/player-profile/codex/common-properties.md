...
- **条目按对象的稳定 `Id` 索引。** 图鉴条目以对应内容条目（`EnemyTemplate` / `PlayerPowerData` / `ItemData` …）的 `Id` 为键——与全库「稳定 `Id` 是一切引用的键」一致（见 `20-systems/common-properties.md`）。绝不用显示名或索引作键。
...
- **解锁是一次性的全量写入（已定案，由 EnemyCodex 确立）。** 触发一次即解锁该条目的**全部词条文案**——**逐项 / 逐招式解锁已否决**。因此解锁状态**只需表达「已解锁」这一态**；「已击败」「使用过 N 次」之类若需要，是额外的计数字段，**不是解锁前提**。Source: `10-handoffs/2026-08-01b-abstraction-levels-combat-numbers-codex-family-and-monetization.md`。
- **EnemyCodex 的触发 = 遭遇（不必击败）。** 失败的战斗同样留下知识。其余四个图鉴的触发未定，见 `_index.md`。Source: `10-handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
