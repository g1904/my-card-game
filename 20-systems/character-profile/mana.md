...
- **战斗中每回合开始恢复至上限（已定案）。** 战斗内，每个回合开始时 `currentMana` **自动恢复到角色当前的 `manaLimit`**（满值）。回合内未用完的 mana **不结转**（每回合刷满即隐含不累积）。Source: `10-handoffs/2026-07-30b-combat-level-intent-and-decision-point-saves.md`。
