...
- **deck 是 CharacterProfile 的一部分。** 卡组、hand、以及打出一张卡的结算，都是单次 run / 单角色的状态；deck 随 run 推进被增 / 删 / 升级。Source: `10-handoffs/2026-07-24-docs-restructure-class-model.md`。
- **抽牌堆 / hand / 弃牌堆 · seeded 洗牌 · deck 变更。** 抽牌、手牌、弃牌的循环，以及洗牌必须由 run 种子（seed）驱动以保证确定性可复现。deck 变更（增卡 / 删卡 / 升级）来自遭遇（如 Exchange / 奖励）。
