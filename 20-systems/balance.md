...
- **成本类型的 element 清单与数值分档未定：** `selectCost` 已定为**定制复合成本类型**、`lifeSpanCost` 为其一个 element（基准 -1）；其余 element（jade / mana / 道具 / 隐藏属性推拉？）、各 element 的数据形态与基准分档、以及 `skipCost` 的数值取向均未定。→ `20-systems/adventure-event/common-properties.md`。Source: `10-handoffs/2026-07-25b-event-cost-fields-capability-flags-and-service-hierarchy.md`。
