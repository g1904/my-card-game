...
> 每个 ante 的分支节点 map、location（地域）、Travel 路由、节点类型路径导航、月圆之夜式菜单、横向滑动选择、篇章 / 境界推进、blind / ante 缩放。

## 意图
> _设计意图，从 handoffs 中提炼。保持更新。_

### Run 结构与篇章 / 境界推进
- 一次 run 的结构为 **realm → chapter → AdventureEvent**。修炼阶梯（炼气 / Qi Refining → 筑基 / Foundation Establishment → 金丹 / Golden Core → 元婴 / Nascent Soul）共四个 realm；一次 run 为**三个 chapter**，每个 chapter 是相邻两个 realm 之间的攀登。Source: `10-handoffs/2026-07-13.md`。
...
### 分支 map 的图编码与节点形态
- **eventOptions 的服务化生成。** 「从当前可用的 AdventureEvent 中选择」由 **future-event-service** 依当前 characterProfile 产出 eventOptions（见 `20-systems/services/future-event-service.md`）。CharacterProfile 以 `List<AdventureEvent>` 持有整段进程（修行历程）；向前是可选走向、向后是面包屑历史。Source: `10-handoffs/2026-07-15-adventure-event-profiles.md` + `10-handoffs/2026-07-25-lifespan-service-refactor-and-legacy-cleanup.md`。
...
- **节点类型路径导航 / 分支 map 形态**：月圆之夜式菜单 + 横向滑动选择已定案，但**分支 map 的具体图形态 / 节点排布 / 导航手感**（几条并行分支、深度、回溯规则）尚未落定。Source: `10-handoffs/2026-07-13.md`。
...
- **eventOptions 与 possibleFutureEvent 图的关系**：服务产出 eventOptions 后，`AdventureEvent` 上原图字段是保留还是被服务态取代，待厘清。→ `20-systems/services/future-event-service.md`。Source: `10-handoffs/2026-07-25-lifespan-service-refactor-and-legacy-cleanup.md`。
