...
> 总则与共享类型见 `20-systems/architecture.md`「API 契约总则」。本服务实现 `IBootstrappable`（启动链第三步：pull + 迁移）。Source: `10-handoffs/2026-07-27b-service-api-contracts.md`。
...
- 本次新增 `rng`（见 `20-systems/character-profile/_index.md`）、`StartContentVersion`、`LastContentVersion` → **bump schema 版本**。
...
- **`revision` 的产生方与语义。** 断线合并语义依赖比较云端与本地基线的 `revision`（单调递增计数？服务端时间戳？ETag？），由谁分配、客户端如何持有基线值，未定——这是**客户端 ↔ 后端协议契约**问题，应同步登记进 `backend-design-documents/open-questions.md`。Source: `10-handoffs/2026-07-27-content-gating-offline-resilience-and-rng-persistence.md`。
