...
- **两层持有模型（大局骨架，细节未定）。** 账号级的 **玩家信息 / PlayerProfile** 跨轮回持久，持有一组 **角色信息 / CharacterProfile**；每个 CharacterProfile 是一次轮回 / 一个角色的状态与历史，对齐 CycleState 概念。life-cycle-service 是操作这两层的服务。
  ...
  - **CharacterProfile（单次轮回）：** `status`（**ongoing | defeated | completed**）、`chapter`（当前篇章）、`Status`（lifeTotal / lifeTotalLimit、currentMana / manaLimit、以及**隐藏属性** 道心 / faith、煞气 / malefic qi、寿元 / lifeSpan）、`List<AdventureEvent>`、`List<CharacterItems>`、**AdventurePlot key points**（剧情进度锚点；完整剧本内容不落存档，存于云端剧本服务，见 `20-systems/services/plot-manager.md`）等。（结构权威见 `20-systems/character-profile/`。）
...
- **寿元按 AdventureEvent 扣减、归 0 → defeated（已定案）。** 隐藏属性 **寿元 / lifeSpan** 是独立于 `lifeTotal` 的寿命预算（炼气起始 100、抵达筑基 +100、抵达金丹 +300、抵达元婴 +500——元婴为游戏终点，该增量无可消耗预算，仅作最后一次数值更新并存档），初始隐藏；**30% 起给定性叙事提示、10% 起给红字数值倒数**（见 `40-ux/screen-flow.md`）。**每完成一个 AdventureEvent，life-cycle-service 按该事件的 `lifeSpanCost` 扣减寿元**（内容侧为正数量值，物化时已取负）；递减到 **0** 即触发「大限将至」，角色置 `status = defeated`。`lifeSpanCost` 是 AdventureEvent 的共有字段（见 `20-systems/adventure-event/common-properties.md`），其分档是**控制篇章时长的主旋钮**（见 `20-systems/balance.md`）。Source: `10-handoffs/2026-07-25-lifespan-service-refactor-and-legacy-cleanup.md` + `10-handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
- **剩余寿元跨篇章结转（已定案）。** 篇章突破时**不清空剩余寿元**：下一篇章的可用预算 = **该篇章增量 + 上一篇章的剩余**（例：第二篇章 = `+100 + 第一篇章剩余`）。因此「省着花」有**跨篇章回报**，寿元成为一条贯穿整个轮回的资源线，而非每章重置的计时器。它是 ChapterManager 在篇章边界的一项明确职责。Source: `10-handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
- **`lifeTotal` 归 0 → `defeated`（已定案 · 第二条终结路径）。** `life` 已定名为 **`lifeTotal`**（角色的生命值，见 `20-systems/character-profile/life-total.md`）；它按战斗失败的道念差被扣减，**归 0 即 `status = defeated`**，与「寿元归 0（大限将至）」并列。二者分工：**寿元按事件流逝，lifeTotal 按失败流逝**。**恢复途径 = AdventureEvent 的 reward**，与等级 / `manaLimit` 同走 `ProfileChangeSpec` → `TryApply` 链路。连带：`DefeatReason` 的 `CombatLost` 作废（输一场战斗本身不终结角色），改为 **`LifeTotalExhausted`**；终结原因收敛为**主动弃置 / 寿元耗尽 / lifeTotal 耗尽**三种。Source: `10-handoffs/2026-08-01b-abstraction-levels-combat-numbers-codex-family-and-monetization.md`。
- **重试上限是基线值，可被付费礼包改写（已定案）。** ADR-0004 的「无限 / 3 / 1」不再是常量：持有 premium bundle 的账号为「**无限 / 9 / 3**」。这使 `RetryChapter` 的上限判定需读账号级的持有状态（落点未定，见待决问题）。见 `20-systems/monetization.md`。Source: 同上。
- **等级成长 = 事件产出（已定案）。** 境界内等级（见 `20-systems/game-progression.md`）由 **AdventureEvent 的 reward 给予**——**不只绑定 Combat / Practice**，也**不只有胜利才给**：失败同样可能有等级产出。它与 `manaLimit` 同属一套「由事件 cost / reward 推拉」的成长体系，走同一条 `ProfileChangeSpec` → `TryApply` 链路。产出的频次与分布未定，见待决问题。Source: 同上。
...
- **篇章存档 · 读档续章 · 重试模型。** 篇章通关即在所达境界落一个**存档点**（如打通炼气→筑基得到筑基存档）；可读档从该境界起始下一篇章。**炼气起手为随机角色，失败可近乎无限重试**；而**落过境界存档的角色，在后续篇章有有限的重试次数**——存档角色是一种会被耗尽的有限资源。**重试上限（四境三篇章 · 基线值）：** 第一章（炼气→筑基）= **无限**；第二章（筑基→金丹）= **3**；第三章（金丹→元婴）= **1**——**持有 premium bundle 的账号为 无限 / 9 / 3**（见 `20-systems/monetization.md`）。挑战成功进入下一境界，不能重试之前篇章。Source: `10-handoffs/2026-07-15b-taxonomy-and-checkpoint-clarifications.md` + `10-handoffs/2026-07-22-online-cloud-combat-and-meta-clarifications.md`。
- **重试计数只有一种：每个角色的篇章重试（已定案）。** 计数**挂在角色上**，且**只有角色进入第二 / 第三篇章后才存在**——第一篇章是**随机生成起始角色**的重试，**不能指定重试同一个角色**，故第一篇章不存在「某个角色的重试计数」这回事。**不存在事件级或篇章级的重试次数上限**：草案中「同一事件重试 < 10 次 / 篇章重试总数 < 30 次 / 超限强制 defeat」**未采纳**，其目的（防退出重进作弊）已由决策点存档达成。Source: `10-handoffs/2026-07-30b-combat-level-intent-and-decision-point-saves.md`。
...
- **`eventEnd` 阶段把 `CombatResult.Spoils`（一份 `ProfileChangeSpec`）连同 `lifeSpanCost`、**战斗失败时按道念差扣减的 lifeTotal**、等级产出与隐藏属性推拉合并为一次 `TryApply`**，从而「一个事件 = 一次事务 = 一个存档点」。combat-service 只**描述**结果（含双方道念），不自行写档。
- **隐藏属性跨档时附一条定性叙事。** `eventEnd` 合并施加隐藏属性推拉；若某属性因此**跨过一个隐藏档位**，则在 `ResolveOutcome` 上附带一条定性描述（不给数字）随事件收口一并呈现。**复用既有链路，无新结构**；档位与文案归 `plot-manager.md`。Source: `10-handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
...
- **决策点的粒度。** 「事件过程按决策点落存档」已定，但决策点具体指哪些位置（事件内每次选择后？战斗内每回合开始 / 每次出牌后 / 每次目标选择后？）未定；粒度直接决定本地写入频率与 push 防抖压力。→ `combat-service.md`、`sync-service.md`。Source: `10-handoffs/2026-07-30b-combat-level-intent-and-decision-point-saves.md`。
- **`AdvanceEventAsync` 的取消触发方。** 取消后的**成本处置已定**（`SelectCost` 不回滚，视同已结算）；仍待定的是「**谁**会取消一场进行中的事件 / 战斗」（玩家主动退出？断线？应用挂起？），以及取消时如何与最近的决策点存档对齐。→ `combat-service.md`、`sync-service.md`。Source: `10-handoffs/2026-07-27b-service-api-contracts.md` + `10-handoffs/2026-07-30b-combat-level-intent-and-decision-point-saves.md`。
...
- **`attemptIndex` 是否还需要（动机已被消解）。** 防 re-roll 派生式 `Hash64(combatStreamSeed, eventId, attemptIndex)` 的原始动机是「退出重进会重掷战斗随机」；**决策点存档 + RNG `State` 持久化已从根上关闭该窗口**。剩下的问题收窄为：**篇章重试（ADR-0004）重开同一篇章时，同名事件是否应换一套战斗随机**——若应，`attemptIndex` 保留并取「篇章重试的第几次」（复用 ChapterManager 的既有计数）；若不应，该派生层可整个去掉。→ 亦见 `combat-service.md`、`20-systems/common-properties.md`。Source: `10-handoffs/2026-07-27-content-gating-offline-resilience-and-rng-persistence.md` + `10-handoffs/2026-07-30b-combat-level-intent-and-decision-point-saves.md`。
...
- **隐藏属性细节：** 属性隐藏、`faith` = 道心、寿元按 `lifeSpanCost` 扣减 / 归 0 → defeated 均已定案；仍待定：**隐藏属性完整清单**（道心 / 煞气 / 寿元之外）、**各自的隐藏档位划分与阈值**（跨档定性反馈的触发依赖它）、增减触发、是否有非境界突破的寿元增长途径、AdventurePlot 树的数据编码（归 `20-systems/services/plot-manager.md`）。Source: `10-handoffs/2026-07-23-adventure-plot-hidden-stats-and-clarifications.md` + `10-handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
- **等级产出的频次与分布（08-01 新增）。** 「等级成长 = event reward、失败也可能给」已定；仍待定：一章内需要多少个「升级型产出」才能把炼气从 1 爬到 13、它们在事件池中如何分布、失败给的等级产出是否弱于胜利。**它与寿元预算的花法互相约束**——事件总数少则升级产出必须密集。→ `20-systems/game-progression.md`、`20-systems/balance.md`。Source: `10-handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md`。
- **重试上限可变后的存档表达（08-01b 新增）。** 上限由付费礼包改写为「无限 / 9 / 3」（见 `20-systems/monetization.md`），故 `RetryChapter` 的判定要读 PlayerProfile 上的持有状态。它落成什么——一个 `CapabilityFlag`、modifier pipeline 的一条具名修正，还是一个独立的 `Entitlement` 字段？→ `profile-service.md`、`20-systems/player-profile/`。Source: `10-handoffs/2026-08-01b-abstraction-levels-combat-numbers-codex-family-and-monetization.md`。
