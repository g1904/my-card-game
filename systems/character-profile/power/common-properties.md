...
- **形状对标 PlayerPower。** `status`（启用 / 禁用）开关、事件触发器驱动的被动修正、capability flag + modifier pipeline 两条生效通道——除非另有陈述，沿用 `../../player-profile/player-power/common-properties.md` 的模型。**「拥有 / 失去」与「启用 / 禁用」是两个正交维度**（失去 = 移出持有列表，而非置禁用）；`status` 落 `bool Status`，record 形态见 `../../player-profile/_index.md`。
...
Source: `handoffs/2026-08-12b-grant-source-per-kind-scope.md` · `handoffs/2026-09-06-status-vs-ownership-encoding.md`
...
- **持有条目侧的 `Status` 一格**：`bool`（true = 启用，默认 true），落 `CharacterPower` record；**拥有 = `characterPower` 的成员资格**，失去 = 移出该列表；**本轮回禁用是第三维**，落 `CharacterProfile.disabledAbility`。**写入经 `ProfileChangeSpec.AbilityStatusChanges`**，门面 `SetAbilityStatus(kind, scope, abilityId, enabled)`（见 `systems/services/profile-service.md`）。record 形态的权威在 `systems/player-profile/_index.md`，本文件**不复述**。
