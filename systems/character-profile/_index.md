...
- **`pastEvent`：修行历程 = `IReadOnlyList<PastEventEntry>`（已定案 · 08-09c）。** 元素**不是 `Resource`**——存的是**定稿实例快照 + 本次结算的最终账**，这是物化模型的直接推论（`AdventureEventData` 是 ContentRegistry 的共享只读单例，痕迹要记的是「这一次走过的那个实例」）。
...
- **`chapterRetry`：篇章重试计数器（已定案 · 08-06 · CharacterProfile 上的新字段）。** 一个**类**，计数第一 / 第二 / 第三篇章各自的重试次数——**因为 ch2 与 ch3 有重试上限**（无限 / 3 / 1，持 premium bundle 为 无限 / 9 / 3，见 ADR-0004）。**它是计数器容器，不是上限持有者**：上限仍按 ADR-0004 的既定纪律读取（可被账号级持有状态改写、凡读取处不得硬编码常量），`chapterRetry` 只答「用掉了几次」。**推论：篇章解锁 / 重新锁定与「剩余重试次数展示」有了确定的数据源。**
  ...
  - **ch1 的角色级计数恒为 0，这不是缺陷（已定案 · 08-06b · 08-10c 修正补位方）。** ch1 重试 = 随机生成新角色（07-30b，**不改写**），故角色级 ch1 计数对每个新角色恒为 0。**「你在炼气段重开了多少次」目前没有字段回答**——账号级统计的首批只有 `TotalCyclesCompleted` / `TotalCyclesDefeated`，后者不区分篇章（见 `systems/player-profile/_index.md`）。这是一个**展示需求**，需要时在 `PlayerStatistics` 上纯加法补一项即可（统计层新增字段零迁移、零后端配合）。**两层口径不同，不是同一个数的两份拷贝**：角色级参与闸门判定，账号级只被读来看。
...
- **`disabledAbility`：本轮回禁用表（已定案 · 08-10c · CharacterProfile 上的新字段 · 与 `pastEvent` / `chapterRetry` / `activeCombat` 平级）。** 08-06b 定下「法则不被强制剥夺，其余一律降级为本轮回禁用」的语义，本字段是它的承载面，覆盖**四类**能力条目（神通 / 法则 / 法宝 / 古宝）。
...
- **`Status` 上的隐藏属性档位与篇章寿元预算（已定案 · 08-12d · 四个新字段）。** 隐藏属性的档位带**回滞**（进入阈值 / 退出阈值不同）⇒ **档位不再是当前值的纯函数，必须持久化**；寿元百分比需要一个**冻结的分母**。
  ...
  - **不进 `PastEventEntry`**：band 设值已在 `AppliedChange` 内、可重放，按 08-09c 判据「重算得出来的不存」⇒ 快照不加字段。
...
- **CharacterProfile 字段结构细节：** 各字段的具体 schema、**隐藏属性完整清单是否还有第四项**、AdventurePlot key points 粒度仍待定。（**隐藏属性的取值域、档位表与阈值已于 08-12d 定案**，见 `systems/services/plot-manager.md`。）→ 见 `systems/services/life-cycle-service.md`、`systems/services/plot-manager.md`。
