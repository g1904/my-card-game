...
- **CharacterProfile / 角色信息（单次轮回）：** 一次轮回 / 一个角色的状态与历史（对齐 CycleState 概念）：`status`（ongoing | defeated | completed）、`chapter`、`Status`（寿元 / mana + 隐藏属性 道心 / 煞气）、`pastEvent: IReadOnlyList<PastEventEntry>`、`magicPack: List<CharacterItem>`、AdventurePlot key points 等。结构权威见 `systems/character-profile/`。
...
顺序：`ContentService.InitializeAsync`（版本比对 + overlay 合并 + 校验，断网降级到基线）→ `AccountService.InitializeAsync`（读本地凭据 + 静默续期）→ **`LoginScreen`（条件步）** → **`ContentService.RefreshFlagsAsync`**（首次 flags 拉取）→ `SyncService.InitializeAsync`（pull + 迁移）→ `ProfileService.Hydrate` → `MainMenu`。三个纯本地服务（profile / life-cycle / combat）**不实现该接口**。这给了「首启不依赖网络下载内容，但进入游戏仍需登录」一个明确落点。

- **登录屏是条件步，不是无条件首屏。** `AccountService.InitializeAsync` 内部先读本地凭据并尝试静默续期，**成功即跳过登录屏**；缺失 / 无效 / 续期失败才呈现它，由玩家提交 `SignInAsync`。静默续期是 account-service 自己的 I/O，放进它自己的 `InitializeAsync` 不新增任何启动步；另立一个「凭据探测步」只是把同一件事多包一层，且那一步仍得调用 account-service 的方法。形态、失效路径与三分支见 `systems/services/account-service.md`「refresh token 的持有与失效」。
- **flags 刷新为什么另立一步、不并进 `ContentService.InitializeAsync`：** flags 端点**需鉴权**，而 content-service 是启动链第一步、跑在登录之前——两者对不上。排在会话到手之后、`SyncService.InitializeAsync` 之前：抽取池必须在轮回开始前正确，而它**失败不阻塞**，放在硬阻塞的 pull 之前不增加任何阻塞风险。语义见 `systems/services/content-service.md`「flags：`ContentEnabled` 的第三层」。

#### 总则 5 —— EventBus：C# 泛型事件 + `readonly record struct` 负载

Godot `[Signal]` 传自定义负载要求负载继承 `GodotObject`，于是**每次广播都分配一个引用对象并经 `Variant` 装箱**——撞上「层与层之间不做隐式装箱 / 转换」与「热路径不分配」。核心循环每步广播 `EventResolved`、战斗内每一次结算广播 `CombatFeedEntry`，这条路径不该分配。EventBus 仍是 autoload `Node`（留在场景树里、可在 `_ExitTree` 做泄漏检查），但对外暴露**强类型 C# 事件**：
...
2. **定位信息取自 `d.Method`，不取自 `d.Target`。** 泄漏发生时目标实例已释放，`Target.ToString()` 无用；`Method.DeclaringType.Name + "." + Method.Name` 是反射元数据，**不依赖实例存活**，直接给出 `CombatScreen.OnCombatFeedEntry` 这样可定位的名字。**因此订阅时不需额外登记来源，`+=` 的惯用形态原样保留。**
3. **触发时机 = 每次屏幕切换完成后**，由编排顶点 game-progression 调一次 `AuditSubscribers()`。屏幕场景是订阅者的绝大多数，切屏正是它们被释放的边界；频率 = 每次切屏一次，**零热路径成本**。**否决「每次 `Emit` 顺带检查」**——`GetInvocationList()` 每次分配数组，`CombatFeedEntry` 在战斗热路径上（它覆盖卡牌结算 / 触发式异能 / 疲劳 / fizzle 四类，频率高于每张牌一次），直接撞「热路径不分配」。**EventBus 自己的 `_ExitTree` 只打一条订阅计数摘要，不做泄漏判定**：此时七个 autoload 服务可能正在同步销毁，`IsInstanceValid` 会对它们误报。
...
public sealed record ProfileChangeSpec                                    // 平级只读列表，逐条按施加语义分列
{
    ...
    public IReadOnlyList<CodexUnlock>          CodexElements   { get; }   // 图鉴解锁：按 (Kind, Id) 的幂等收录
    public IReadOnlyList<ItemChargeElement>    ItemElements    { get; }   // 道具次数：按 (Scope, ItemId) 选定实例后的带符号增量
    public IReadOnlyList<ItemUseEntry>         ItemUseElements { get; }   // 战斗外使用痕迹：序列尾部只追加
    public IReadOnlyList<AbilityStatusAssignment> AbilityStatusChanges { get; } // 能力启用开关：按 (Kind, Scope, AbilityId) 的绝对置值
    public IReadOnlyList<AchievementProgressElement> AchievementElements { get; }    // 成就进度：按 AchievementId 的带符号增量，按 Target 钳制
    public IReadOnlyList<AchievementTierAward>       AchievementTierElements { get; } // 成就档位水位：按 GroupId 的绝对置值
}                                        // CodexUnlock / CodexKind / ItemChargeElement / 两条成就 element 的定义见 systems/services/profile-service.md
                                         // ItemUseEntry 的定义见 systems/character-profile/_index.md
public readonly record struct AbilityStatusAssignment(  // 绝对置值：把已持有条目的 Status 置为 Enabled
...
public readonly record struct AbilityChangeElement(
    AbilityChangeOp Op, AbilityCarrierKind Kind, AbilityScope Scope,
...
public enum CostKey         {                                             // 资源族 element 键；15 值，逐一在 ResourceElements 表中占一行
                              ...
                              LifeSpan, SpiritStone, ImmortalJade, ManaLimit,
...
public enum ModifierOp      { Add, Scale }                    // Scale 的 Value 是万分比增量（-2000 = −20%）
public readonly record struct ModifierEntry(ModifierKey Key, ModifierOp Op, int Value);
                                                              // 一条具名修正的声明形态；合并算法见 systems/services/profile-service.md
...
public enum AbilityCarrierKind { Power, Item }                            // 能力挂在哪一族载体上
...
// —— 全部 flag 恒为增益向是「union 即全部叠加规则」这条不变式的前提，见 systems/services/profile-service.md
public enum HiddenStat      { Faith, Bloodlust }
...
public enum OutcomeRuleKind  { FixedResource, GrantFromPool, DeckOperation }
public enum OutcomeDirection { Gain, Loss }                   // 取负发生在物化组装，与 SelectCost 同处
public enum ExperienceGrade  { None, Minor, Standard, Major } // 档 → 值的映射住平衡资源，内容侧不落裸数字
public enum HiddenStatGrade  { None, Minor, Standard, Major } // 同上；与 ExperienceGrade 是两张表，不得互换
...
- **`HiddenStatDirection` 与 `OutcomeDirection` 分立，不复用。** `Gain` / `Loss` 是价值判断词，其既有唯一用法绑在 `LifeSpan` / `ManaLimit` / `SpiritStone` / `ImmortalJade` 四个 key 上——那里「得到 = 数值升」两轴重合。移到隐藏属性上两轴当场分离：`(Bloodlust, Major, Gain)` 有「煞气 +10」与「玩家获益 ⇒ 煞气 −10」两个自洽读法，`.tres` 里读不出作者想的是哪一种。`Raise` / `Lower` 沿**数值轴**命名，对两个属性一律无歧义。过载的第二笔代价是「见到 `OutcomeDirection` ⇒ 它作用在一个 `FixedResource` 资源量上」这条恒真句失效。
...
- **`Stat` 就取 `HiddenStat` 本身，不另立 `PushableHiddenStat` 一类子枚举。** 枚举成员即全部可推拉的属性，取值域无需再由校验收窄；`HiddenStat` 正被 `HiddenStatBandData.Stat`、`PlotCondition.Kind == HiddenStatBand`、EventBus 的 `PlotThresholdReached` 三处使用，多一个近同义枚举会让「该用哪个」成为每次新增字段都要回答一遍的问题。
...
**枚举的**类型名**不参与序列化、不在契约内；上面那条冻结纪律只约束成员名（承重）。** 存档与上行负载里落的是成员名（或整数 code），读回时按目标字段的静态类型解析，类型名一个字节都不出现 ⇒ **重命名一个枚举类型是零迁移的**，不 bump `schemaVersion`、不与后端同批改。不写明这一条，「枚举以成员名逐字序列化 ⇒ 重命名即破坏性契约变更」会被读成「连类型名也动不得」，一个纯命名订正就会被误判为契约变更而永远拖着。
...
**`AbilityStatusChanges` 承载四类持有条目上的 `Status` 开关，语义是按 `(Kind, Scope, AbilityId)` 的绝对置值。** 它按三级判据的 ① 分列，与 `AbilityElements` 的差别落在五面上：**施加语义**（集合成员增删 vs 已有成员上的字段置值）· **`Source`**（前者强制携带并校验合法子集、`Unknown` 即整批拒绝，后者无落点——开关不是一次授予，没有来源可言）· **`PairKey` / `Duration`**（前者承载置换配对与禁用时长，后者两格恒空）· **幂等的含义**（重复 `Grant` 是空操作，重复置值是覆盖）· **键与载荷的形状**（三元组键 → 单个 `bool`）。把一个不改变持有、无 `Source`、无 `Duration`、无 `PairKey` 的操作塞进 `AbilityElements`，等于让该列的载荷有一半字段对某个 `Op` 恒空——正是 `ChangeElement` 拒绝加可空字段时否决过的形状。**同族先例是 `SettingChanges`**：玩家自己拨的开关 · 按固定键绝对置值 · 恒不走 modifier pipeline，三面逐字同构；两者仍分列，因为作用对象与键空间不同（一个是 `GameSetting` 的固定枚举键，一个是持有条目的三元组键）。**恒不走 modifier pipeline**：一条法则若能改写玩家自己拨的开关，等于内容替玩家做主。**它与 `StatusChanges` 无关**——后者绑定 `CharacterProfile.Status` 上的数值型规则字段，名字撞车、语义无交集。施加与失败语义见 `systems/services/profile-service.md`，四类 record 的 `Status` 形态见 `systems/player-profile/_index.md`。
...
| `AchievementCompleted` | `(string AchievementId)` | profile |
...
**剧本层只广播 `PlotThresholdReached` 一条。** 分支揭示走宿主服务门面的只读查询 `TryGetPlotSegment`（`PlotSegment` 是完整实例、进不了负载；且揭示的目的是等玩家输入 = 纪律 3 的「询问」）；分支选择与 key point 推进零跨系统消费方，不广播。成就采集的 EventBus 那一支若确有剧本条件，按 `PlotArcAdvanced`（广播者 future-event 代 PlotManager）补一行 —— **它当前不在本表内**，形状与触发条件见 `systems/services/plot-manager.md`「事件面」。补一行是正确加法；另开一条上报旁路不是。
