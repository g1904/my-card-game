...
- **eventOptions 的生成 / 加权 = 一条十步管线（适用范围 = 常规批）。** 下述十步描述**常规批**；**闸门批在 ① 之前短路**——`LocationEventCount >= EventCountLimit` 成立即走上方的 Travel 段伪码，整批归 Travel，不进本管线（它的取池链是邻接集合、不经 `AllEnabled()`，硬塞进同一条管线会模糊那条明写的例外）；`Priority = 1` 收窄批同理由 `systems/adventure-event/_index.md` 的既有规则给出。
  ...
  - **⑦ 的条目权重同时是「失去能力」频次预算的全局微调旋钮**——该预算是内容编排口径，目标频次与旋钮位置见 `../player-profile/player-power/_index.md`；本管线不为它新增任何机制或参数。
...
- **敌人物化 = 一条五旋钮管线，输入固定、顺序固定、产物落存档。**
  ...
  ② 赋级          ← 角色全局等级 ±2 带 + 权重表（BandFor(chapter)，见 systems/balance.md）
                    （Tier == Finale 旁路：不掷骰，直接指派 角色等级 + BandFor(chapter).FinaleDiff）
...
- **赋级的合法区间 = 角色当前等级 `±2` 的对称带（三章统一 · 承重）。** 物化赋级落在 `[角色等级 − 2, 角色等级 + 2]` 内，在全局序 **1–22** 上截断。
  ...
  - **Finale 是带内的一条指派旁路：不掷骰，取 `角色等级 + BandFor(chapter).FinaleDiff`。** 旁路只是绕过掷骰这一步，**不绕过带**——`FinaleDiff` 由加载期校验锁在 `[1, Upper]` 内且保证 `本境界末级 + FinaleDiff <= 22`，故指派结果恒落在带内、恒越阶。**它不是传给赋级函数的区间覆盖参数**（推论 ③ 因此逐字成立），而是 Finale 这条既有指派通道自己的取值来源；本服务仍只读「当前篇章的带」这一个概念。取值与校验形态见 `systems/balance.md`。角色在 Finale 时刻恒处本境界末级（抬升条件即 `level == 该境界末级`），故指派值不需要读任何额外状态。
