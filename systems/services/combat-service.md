...
- **战斗模型 = mana（出牌）+ 道念（计分与胜负）。** 本服务维护**双方各自的道念（momentum）**作为胜负标尺：**道念高者胜**；`currentMana / manaLimit` 为出牌资源，mana **无曲线**、**每回合开始自动恢复至 `manaLimit`**。**寿元 `lifeSpan`（单值，无上限字段）在战斗过程中不被读写**——失败时才在收口时刻按「角色道念 − 敌人道念」的差值乘该篇章的 `lossPerMomentum` 扣减。炼气基线寿元 1000、mana 5/5。见 `systems/scoring.md`、`systems/character-profile/life-span.md`、`mana.md`、`systems/adventure-event/combat/`。
- **战斗是定长的：回合数由本场遭遇给定（`Practice` 8 / `Standard` 10 / `Finale` 12）。** 一场战斗**打满 `EncounterSpec.TurnLimit` 个回合**（双方合计，交替进行，「回合」= 单方的一次行动轮），随后按 `VictoryRule` 比道念定胜负。**不设提前终止**（无道念阈值胜利、不以卡组耗尽终止）。**推论：TurnManager 是一个定长循环**（`for turn in 1..TurnLimit`）而非动态终止判定——状态机形状因此确定，且每场战斗的时间开销可预测。**定长不等于常量**：长度是遭遇参数，见下方「回合数与胜负判据是遭遇参数」。
- **平局 = 只发基础奖励（`Standard` 档）。** `Standard` 档打满 10 回合后道念相等时：**不判负、不扣寿元**，玩家**只获得该事件的基础奖励**（道念差为 0，故无任何厚度加成）。因此 `CombatOutcome` 需要第三个胜负态 `Draw`，且它在收口上落在「胜利侧的最薄一档」——与「道念差是双向刻度」自洽：差值为 0 就是两侧都不加码的那个原点。**`Draw` 只在 `Standard` 一档可达**：另两档 `WinMargin` 为 0，相等即判胜（`Practice` 点到为止 / `Finale` 不落后即通过），`Draw` 分支在那里恒不成立。
...
- **奖励由本服务计算，且「获取奖励」是战斗流程的一部分。** 结算量不由 life-cycle-service 拿着 `CombatResult` 的双方道念在 `eventEnd` 再算——**combat-service 按战斗结果算完**，包括按道念差决定的奖励厚度与寿元扣减。**推论 ①：`RunCombatAsync` 的流程尾部含奖励环节**——10 回合打完后还要走「胜负判定 → 计算奖励 →（若有可选奖励）等玩家选择 → 收口」，随后才返回 `CombatResult`；它因此仍是形态 C，只是尾部多了一个等待玩家输入的阶段。**推论 ②：不违反「一个事件的收口是一次事务、一个存档点」**——本服务只**计算并确定**奖励，产出的仍是一份 `ProfileChangeSpec`（`Spoils`），真正的写入照旧由 life-cycle-service 在 `eventEnd` 合并为一次 `TryApply`。**分工 = 计算归战斗、施加归生命周期。** **推论 ③：本服务交出的 `Spoils` 内的授予一律记 `Source.CombatReward`**——授予来源的分野判据是「谁组装出这条 element」而非「属于哪类事件」，故一个揭示出战斗真身的 Explore 选项，其战利品同样记 `CombatReward`；唯一例外是 `Finale` 胜利时由道统残卷发放的那一路，走 `Source.FinaleWin`。见 `systems/common-properties.md`。
...
- **手牌上限是一条恒定不变式，不设弃牌机制。** **手牌在任何时刻都不得超过上限**——**没有时间限制，也没有「结束阶段弃到上限」这类必须弃牌的机制**（结束阶段因此只做「触发『回合结束时』→ 清理回合内状态」）。**上限值 = 7，是一个不可被任何编排面改动的全局常量**——不进 `EncounterSpec` 覆写组、不进 `EncounterTighten` 增量面（见 `systems/balance.md`）。 **推论：约束点落在会让手牌增加的时刻**（抽牌、以及任何「加入手牌」类效果）。
...
- **回合数与胜负判据是遭遇参数，不是常量。** **`Standard` 档 = 10 回合、道念高者胜**；**`Practice` / `Finale` 档可改写回合数与胜负条件**（前者更简单、后者更难，对位 Balatro 的 small / big / boss blind）。**推论：TurnManager 仍是定长循环，但长度来自本场遭遇的配置**，且胜负判据是一个可替换的判定而非写死的比较。**承载已定：`EncounterSpec.TurnLimit`（`Practice` 8 / `Standard` 10 / `Finale` 12）与 `EncounterSpec.VictoryRule`**（三档 `WinMargin` 0 / 1 / 0），见下方「API 面」的 `EncounterSpec` 与「决策」段。
...
- **战斗内的一切写入经 ProfileManager。** 耗 mana、消耗道具、获得战利品、以及**收口时按道念差扣寿元** 都走 `profile-service.ProfileManager.TryApply(...)`——本服务不直接改 CharacterProfile 字段。**道念本身是战斗内的运行态**（活在 `CombatSnapshot` 里），战斗结束即消失，不落 CharacterProfile。
...
- **AI 兜底算法 = 单层（1-ply）加权效用评分 + 确定性 argmax（承重）。** 一次决一个动作，执行到栈清空后重新组装候选集再决下一个——**栈结算会改变局面**（连锁触发、下限 0 截断、条目落场 / 离场），一次性规划出的后续动作在执行到时的合法性与价值都可能已变。候选集：
  ...
  { MomentumGain, MomentumDenial, ManaEfficiency, BoardPresence, Removal,
    AmbushCaution, HandRetention, KeyCardBias, ClosingUrgency, ItemEagerness }
...
- **AI 决策是「当前局面 + `combat` 子流」的纯函数（承重）。** 不得依赖真实时间、帧序或未持久化的隐藏记忆；**随机只取 `combat` 子流，不再派生新流**——而**兜底与定制策略均零随机消耗**（平手按确定性字典序打破：`−score` → 动作种类序 → 主体 id 的组装序），故这条约束当前为空，保留它是为日后可能引入的随机化权重项。理由：敌人回合内部不落决策点，**D5 一个点即覆盖整个敌人回合，它是一段可确定性重放的区间**——玩家在敌人回合中途退出时整段被重放，任何不在存档里的私有记忆都会让重放分叉。**输入面限对称可见信息**：战场上双方全部**面朝上**条目（含 `OwnerSide`）+ 己方面朝下条目 · 双方道念与回合数 · 对手埋伏**计数** · 对手手牌**张数** · 自己的手牌 / 卡组 / 本场可用道具；**不读玩家手牌内容与抽牌堆顺序**——这一条做到 `ADR-0013` 第 1 级：AI 读的是 `viewerSide` 为敌方的 `CombatSnapshot`，而 `SideSnapshot.HandCardInstanceIds` 恒只有 viewer 己方非空、抽牌堆只给张数，玩家手牌内容与牌序**在类型层根本没有承载它们的字段**。**AI 的目标选择照旧不产生决策点**，定制策略不改变这一条。
...
- **先后手由 `EncounterSpec.FirstSide` 决定，未指定则随机。** `EncounterSpec` 带一个可空字段 **`FirstSide: Side?`**（`null` = 未指定），由 **future-event-service 在物化 eventOption 时写入**——「剧情指定先手」= **内容侧在事件模板（`AdventureEventData`）上直接编排**；**未指定时由本服务用 combat 子流掷**，故同一 seed 复现同一个先后手。**推论 ①：本服务只读该字段、不问来源**——与既有「遭遇参数收进 `EncounterSpec`」（`TurnLimit` / `VictoryRule` / 抽牌覆写）完全同形，**不新增对 future-event-service 的运行时依赖**。**推论 ②：与「不设先后手抽牌差」不冲突**——那条说的是不做补偿（先手 tempo 优势在打满回合比总量的结构下不存在），本条说的是谁先动。**推论 ③：掷点消耗的随机落在 D0 之前**，随参战方组装一并反映进持久化的 RNG `State`，退出重进不会改变先后手。
...
- **抽牌堆不重洗，抽空即疲劳（承重）。** **抽牌堆抽空即为空，弃牌堆不回流**——没有「抽牌堆空时由弃牌堆重洗补充」这条规则。**抽牌堆为空时每尝试抽一张牌，抽牌方失去 1 点道念**（一次抽 N 张即失去 N 点）。
  ...
  - **推论 ④：`DeckModule` 没有「弃牌堆整堆回流重洗」这条代码路径**，seeded 洗牌只发生在参战方组装时的一次初洗。**卡牌效果可经 `MoveCard` 把有限张牌置于所声明一侧的抽牌堆顶 / 底**（方位由 `MoveCardEffect.Side` 一格声明，两端同侧，见 `systems/character-profile/deck/common-properties.md`）**，插入位置不掷随机**（随机位入堆未开放）；`Selection = Random` 的**源牌**选择照常走 `combat` 子流，那是选哪张的随机、不是插进哪里的随机，两者互不干涉。**抽牌本身因此仍零随机消耗**，「两侧牌序互不打乱」原样成立（见上方「确定性」）。
...
**栈条目**：`stackEntryId` · `kind { PlayedCard, TriggeredAbility, ActivatedAbility, UsedItem, Fatigue }` · `controllerSide`（**决定这次目标选择是否产生决策点**）· `sourceInstanceId` · `sourceEntryId` · `abilityId` · `itemId` · `chosenTargets`（`TargetRef[]`，按槽位顺序）· `targetState { Resolved, AwaitingChoice }`。**栈内位置由数组顺序承载，不落 `position` 字段**——索引即位置，另存一个序号是可以不一致的冗余。

- **`itemId` 与 `kind == UsedItem` 互为双向不变式**：`UsedItem` 条目的 `itemId` 非空，其余四个成员恒空（读档期可断言）。它**不可由三个来源格推导**——道具不是 `CardInstance`（无实例 id）、从不进场（无载体条目 `entryId`）、没有宿主 `AbilityData`，`sourceInstanceId` / `sourceEntryId` / `abilityId` 对它**全部恒空**；而结算要按它解析 `ItemData.CombatUseEffects`、战报要按它显示用的是哪一件道具。
- **`UsedItem` 是独立成员，不复用 `ActivatedAbility`。** 复用会让 `sourceEntryId` 与 `abilityId` 两格同时变成可空，「`ActivatedAbility` ⇒ 有载体条目、有异能主体」这条不变式被整条抹掉，`ActivateAbility` 的「`SubjectId` 取 `entryId` 因为它是唯一寻址」在该成员上也不再成立——那不是加一个分支，是让一个成员失去形状。三个枚举（`CombatActionKind` 我发起了什么 / `StackEntryKind` 栈上这条是什么来源 / `CombatFeedKind` 战报这条是什么事）是同一条链路的三个切面，在「用道具」这一格上必须对齐。
...
**读档校验（强制，六个检查点全部命中）**：① `eventInstanceId` 与当前进行中的 `EventOption` 不一致 → `PushError` + **拒绝恢复该战斗**；② `cardId` / `sourceId` / `abilityId` / **栈条目的 `itemId`** 解析不到 → `PushError` 并报出 id（**注意 `Get(id)` 不过滤 `ContentEnabled`**，故战斗中途某条目被线上关闭仍能恢复——这正是「读取侧不过滤」的用武之地）；③ `pending.stackEntryId` 在 `stack` 中找不到 → `PushError`（内部一致性破损）；④ **三区 `Id` 序列的并集 ≠ `instances` 全集 → `PushError`**（闭集不变式的自校验，是「无凭空生成的牌」买来的一条免费断言）；⑤ `counters` 某键的值为负、或 `UsesThisCombat < 0` → `PushError` + 抛，带 `entryId` / `cardInstanceId` / `itemId`（`counters` 两处落点都覆盖；不可能态，与 ③ ④ 同属内部一致性破损）；⑥ `CombatItemSave.ItemId` 不在该侧「本场可用道具」的重建结果内 → `PushWarning` + **丢弃该条**、不阻断恢复（多半是 `disabledAbility` 或 `UsableScene` 中途变化；「本场可用道具」本就是按当前状态重建的派生项，丢弃一条已不可用道具的计数无副作用）。

> **`counters` 键的 `abilityId` 段走 ② 而非 ⑥。** 两者形态相似但语义不同：⑥ 丢弃的是一条**内容仍在、只是本场不再可用**的记录；键解析不到则是真悬空（内容被删或键被写错），与 `sourceId` 悬空同档致命。
>
> **`itemId` 按落点分档：栈条目的走 ②，`CombatItemSave.ItemId` 走 ⑥。** 同一个 id 在两处的失败语义不同——栈上那条是**正在结算的事实**，解析不到即内容被删或键被写错；`items` 里那条只是**本场计数**，其道具本场不再可用是预期内的状态变化。故一条 `UsedItem` 栈条目在它对应的 `CombatItemSave` 被 ⑥ 丢弃时**照常结算**：被丢弃的是计数，不是内容。

**版本化**：本块属 `schemaVersion` 1，登记见 `systems/services/profile-schema-versions.md`。

**本块新增的字段共两格：战场条目的 `amount` 与栈条目的 `itemId`** —— 计数器与道具运行态全部落在既有的 `counters` / `Counters` / `items` 上。量级都可忽略：`amount` 为 **≤ 4 字节 / `Transient` 条目**，`itemId` 只在 `kind == UsedItem` 的栈条目上有值（一场至多几条），相对单次决策点 2–4 KB 的既有量级不成比例；当前无线上存档 ⇒ **空迁移**，故下述量级与迁移面不受影响。**两格都是在内容清单为空时先行铺下的**，取舍同一条：加它的成本此刻恒为零，不加的成本在第一条用得上它的内容落地时可能已不为零（届时 schema 或已上线，加格不再是空迁移）。`kind` 枚举增员本身不加字段——多的只是一个取值。
...
Source: `handoffs/2026-08-22-card-counters-api-and-key-space.md` · `handoffs/2026-08-26b-combat-substream-arbitration.md` · `handoffs/2026-08-26d-activate-ability-contract.md` · `handoffs/2026-08-28-item-use-effect-face-and-carrier-kind.md` · `handoffs/2026-08-30-stack-entry-kind-for-item-use.md`
...
Source: `handoffs/2026-08-03-battlefield-stack-hand-limit-and-power-item-naming.md` · `handoffs/2026-08-04b-mtg-loanwords-card-types-and-intent-snapshot.md` · `handoffs/2026-08-10c-ability-disable-replacement-and-player-statistics.md` · `handoffs/2026-08-17h-profile-field-schema.md` · `handoffs/2026-08-19-profile-change-spec-gaps.md` · `handoffs/2026-08-22-combat-runtime-counter-persistence.md` · `handoffs/2026-08-22-card-counters-api-and-key-space.md` · `handoffs/2026-08-25-combat-presentation-and-action-result.md` · `handoffs/2026-08-26-storage-pack-two-layer-view-and-combat-holdings.md` · `handoffs/2026-08-26c-enemy-ai-strategy-shape.md` · `handoffs/2026-08-26d-activate-ability-contract.md` · `handoffs/2026-08-30-stack-entry-kind-for-item-use.md`
...
| **用道具** | A | `ActionResult UseItem(string itemId, IReadOnlyList<TargetRef> targets)` | 业务失败（不在本场可用道具内、本场配额用尽、充能耗尽、mana 不足、目标非法）→ `ActionResult`，绝不抛 |
...
public sealed record EncounterSpec(               // sealed record 而非 struct：字段多、含引用类型、落存档、非热路径
    ...
    int?              DrawPerTurn,                 //   双方同用一组值（起手 / 每回合抽牌）
...
public readonly record struct CombatResult(
    ...
    int               EnemyMomentum,      // 收口时敌方道念；二者之差：胜 → 奖励厚度，负 → 寿元扣减
    int               RemainingLifeSpan,  // 收口扣完之后剩余的寿元（战斗过程中不读写它）
...
public sealed record CombatSnapshot(              // 只读视图，供 ViewModel 组装与 AI 决策；不落存档
    ...
    SideSnapshot Enemy,
    IReadOnlyList<BattlefieldEntryView> Battlefield,  // 单一记录，条目自带 OwnerSide，呈现层分区渲染；对侧 faceDown 条目整条不入列
    IReadOnlyList<StackEntryView>       Stack,        // 栈顶在前
    PendingTargetRequest? PendingTarget);         // 结算挂起中的「请选目标」；仅 pending.controllerSide == ViewerSide 时填充，否则恒 null
...
public sealed record SideSnapshot(
    ...
    int    AmbushCount,                           // 该侧 faceDown == true 的战场条目计数；双向对称：只给计数不给内容
...
- **`ActionResult` 是玩家动作的统一返回类型，不专属出牌。** 出牌 / 用道具 / 结束回合三个方法各返回一份，`ProvideTarget` 返回它所续报的那次动作的同一类型。它承载的是「**我发起的这次动作，服务受理了没有、结果是什么**」；`Kind` 决定 `SubjectId` 怎么读。
  ...
  - **`UseItem` 的次数消耗即时经 `ProfileManager.TryApply` 写入，本方法不直接改 Profile 字段**——法宝一侧写 `CharacterProfile` 的 `CharacterItem.Charges`，古宝一侧写 `PlayerProfile`（两者的载体都是 `ProfileChangeSpec.ItemElements`，见 `systems/services/profile-service.md`），本场配额落 `CombatItemSave.UsesThisCombat`。**战斗内使用不写 `pastItemUse`**：它发生在事件之内（`activeEvent != null`），账已由该事件的 `AppliedChange` 承载。三条拒绝理由因此各有落点：不在本场可用道具列表内 → `ItemNotAvailable`（含被 `disabledAbility` 过滤掉的）· Profile 侧充能归零 → `ItemChargesExhausted` · 本场配额撞上 `ItemData.MaxUsesPerCombat` → `ItemUsesThisCombatExceeded`。**用道具与出牌完全同窗口**（自己回合的行动阶段、栈为空），故前三条通用拒绝理由照常适用。
    ...
    - **入栈**：`kind = UsedItem` · `controllerSide` = 使用方 · `itemId = ItemData.Id`（`UsedItem` 唯一的来源格）· `sourceInstanceId` = `string.Empty`（道具不是 `CardInstance`）· `sourceEntryId` = `string.Empty`（道具从不进场）· `abilityId` = `string.Empty`（无宿主异能）· `chosenTargets` = 传入列表 · `targetState = Resolved`。
    - **扣费 = 压栈那一刻扣 `sides[controllerSide].currentMana`，`fizzle` 不退，且不经 `ProfileManager`**——与 `ActivateAbility` 逐字同款（`currentMana` 是回合内运行态、不是 Profile 字段），按 `controllerSide` 解析而非写死玩家侧。`currentMana < ItemData.ManaCost` → **`InsufficientMana`（复用，不另立）**；`ManaSpent` 填 `ManaCost`。次数消耗与 mana 是两条互不代替的账：前者写 Profile / `UsesThisCombat`，后者只动本回合的 `currentMana`。
    - **敌人侧不经本方法**：EnemyManager 在自己回合内自行决定用道具、走内部路径，**不产生 `ActionResult`**（没有调用方），入栈填法与扣费同上，照常广播 `CombatFeedEntry`。不为敌人另开 API。
    - **`card.played` 不由 `UseItem` 广播，也不为「使用道具时」新开时点。** 道具在战斗内以 `CardType.Item` 呈现，但它不是被打出的牌——把它并进 `card.played` 会让「打出一张牌时」的监听者被一件道具意外唤醒。新开一个时点则要连带给 `SubjectKind` 增一档并扩 `TriggerFilter` 的相容校验矩阵；时点表随广播点一同增长，当前没有内容需要它，日后要开是纯加法（一行常量 + 一处广播点 + 一档 subject）。
...
- **对侧的面朝下条目整条不入 `Battlefield`（第四条填充纪律）。** 组装时按视角过滤：`entry.faceDown == true && entry.ownerSide != ViewerSide` 的条目**不进列表**，而不是保留壳条目再把内容格置空。置空 / 哨兵值的形态把泄漏防线从结构降为约定——内容格在类型上仍然存在，AI 与 ViewModel 读 `sourceId` / `keywordId` / `amount` 前都要先判 `faceDown`，漏判无人发现，且列表也能数出对侧埋伏数、与 `AmbushCount` 形成两个口径。整条剔除使这一项与手牌内容同档：对侧那份内容**在类型层根本不存在**。另立一个对侧专用条目视图同样不取——与「`SideSnapshot` 单类型，不拆己方 / 对方」「不为 AI 另立第二个投影类型」两条形状相抵，ViewModel 与 AI 各多一条会各自漂移的路径。
  - **不变式（组装期可断言）：** `Battlefield` 中不存在 `FaceDown == true && OwnerSide != ViewerSide` 的条目 ⇒ **视图内 `FaceDown == true` 恒指观察方己方的埋伏**。
  - **`BattlefieldEntryView` 保留 `FaceDown` 一格**：己方埋伏要靠它决定折叠 / 逐条渲染与埋伏标记的呈现，删掉它己方那一半就没形状了。
  - **被剔除的条数由 `SideSnapshot.AmbushCount` 承载，公开面零损失**——对手本就只该知道「有几张」。`AmbushCount` 的定义按 `faceDown == true` 收口（而非按次类型 `enchantment.ambush` 计数），与过滤判据**逐字同源**：两处口径各写各的，一旦出现一个非埋伏的面朝下条目，列表少一条而计数不变，且没有任何机制发现。**不改名为 `FaceDownCount`**：「埋伏计数」已是 `ux/combat-ux.md` 的必做项、也已进 `terminology.md`，收益只是命名精确，代价是三处措辞连锁改动 + 玩家可见词汇分叉。
  - **`Battlefield.Count` 不等于场上条目总数。** 想按列表长度算「场上有几个条目」的消费者须补上对侧的 `AmbushCount`；**当前无此消费者**（AI 的十项 term 与 UX 战场区都按 `OwnerSide` 分区读）。这一点必须写下来，否则日后有人把列表长度当总数用。
  - **视图内的 `entryId` 引用不得假定可解析。** 栈条目的 `sourceEntryId` 与 `CombatFeedEntry.CauseEntryId` 链上的坐标都可能指向不在本视图 `Battlefield` 列表中的条目——被视角过滤剔除，或已离场进弃牌堆（埋伏触发后即进弃牌堆）。呈现层解析不到时按「该条目已不可见」处置，不报错。「引用恒可解析」这条假设本就不成立，视角过滤只是让它更早发生。
  - **前提：对侧 `faceDown` 条目永不进 `LegalTargets`。** 否则 UI 会拿到一个自己列表里没有的 `entryId` 去高亮。该前提由 `TargetSlot` 侧的一条加载期闸机械化（见 `systems/character-profile/deck/common-properties.md`），不靠散文纪律——纪律的失效方式只会在内容侧真用了 `IncludeFaceDown` 的那一天暴露。
- **`PendingTarget` 同样条件填充：仅当 `pending.controllerSide == ViewerSide` 时填充，否则恒 `null`。** 与 `HandCardInstanceIds` / `UsableItemIds` / `ActivatableAbilities` 三条齐平，凑成四条同型的填充纪律。`PendingTargetRequest` 的 `SourceCardId`（呈现用，「埋伏·XX 需要一个目标」）与 `LegalTargets` 承载的都是**挂起方**的内容；`CombatSnapshot` 是双视角的单一投影，`ViewerSide` 取敌方时这两格上承载的正是玩家侧的牌。**不以「AI 只在自己回合读快照、彼时栈空」这条性质代替**——那是时序保证而非结构保证，且它依赖一条从未成文的时序前提。
- **`StackEntryView` 须含 `SourceCardId`，这是该视图上第一格成文的字段。** 该类型此前只在 `CombatSnapshot.Stack` 一格被引用，**字段面整体尚未成文**；本条只定这一格，其余格待该视图（连同 `BattlefieldEntryView`）一次性成文时补齐，届时须与本格合并而非另立。存档侧的栈条目字段面是另一件事，不可混用。
  - **用途 = 揭示时刻的承接面。** 对手的埋伏被触发时玩家从哪里看到它是哪张——**不给战场条目加「已翻面 / 已揭示」态、不加任何战场条目字段**。埋伏触发即压栈，其可观测面由该次触发的栈条目与 `CombatFeedEntry(Kind = AbilityTrigger)` 承担，与「敌方启动的可观测性由飘字与战报承担」同一条纪律。而把 `SourceInstanceId` 解析成「哪张牌」在对侧视角下没有解析通道，故须直接给出 `CardId`。
...
- **`EncounterSpec` 的可空覆写组 = `InitialDraw` / `DrawPerTurn` / `EnemyManaLimit` 三格，`null` = 取 `CombatRulesData` 的默认值。** 它们与 `TurnLimit` / `VictoryRule` 属同一档遭遇参数旋钮——「更宽容的 `Practice`」最自然的形态之一就是多抽一张。取值与逐格理由见 `systems/balance.md`；**疲劳量与手牌上限刻意不在覆写组内**（同处给出理由）。
  - **手牌上限不在覆写组内，任何编排面都改不动它。** 它恒取 `CombatRulesData` 的全局常量 `7`——**模板（`AdventureEventData`）不能覆写它，剧本（`PlotModulation.Tighten`）也不能收紧它**。理由：撞上限**不丢牌**（第 8 张退回抽牌堆）⇒ 压低它实为一次条件性的牌流收紧，与 `InitialDraw` / `DrawPerTurn` 两格直接旋钮重叠，而玩家读不出这次少抽来自哪里。
  - **本服务只见定值，不知道剧本存在。** 前两格连同 `TurnLimit` / `WinMargin` 可能已在物化期被剧本的 `PlotModulation.Tighten` 收紧（形态与合并算子见 `systems/services/plot-manager.md`）；`EncounterSpec` 产出即定稿并落存档，**消费侧不回查模板重算**。
...
- **神通候选的两条口径。**
  - **档位闸：`Practice` 档整族不进候选**（见上）。这是**唯一**一处让三档生成路径产生族级差异的地方。
  - **候选中出现已持有的神通 → 直接排除。** 与功法侧「已持有 → 直接排除，不折算为升阶」同款：神通没有层数维度，重复授予无表达；换一条同 `Rarity` 的神通是**置换**这条独立通道（同池判据见 `systems/player-profile/player-power/_index.md`——`(CarrierKind, Scope)` 全同 + 同 `Rarity` + 排除已持有），混进奖励抽取会让两条通道互相污染。
...
| `CombatFinished` | `(CombatOutcome Outcome, int CharacterMomentum, int EnemyMomentum, int RemainingLifeSpan)` |

**`CombatFeedEntry` 是喂给呈现层的统一结算事件流，六类情形共用一条广播：卡牌结算 · 启动式异能 · 触发式异能 · 用道具 · 疲劳扣减 · fizzle。** 一个事实只留一条广播——飘字 / 战报收起态单行 / 战报展开态因果树三个消费者读的是同一条流，不各自去别处捞。

```csharp
public readonly record struct CombatFeedEntry(
    CombatFeedKind Kind,             // CardPlay | AbilityActivation | AbilityTrigger | ItemUse | Fatigue
    ...
    string         SourceId,         // CardId（CardPlay）/ AbilityId（AbilityActivation | AbilityTrigger）/ ItemId（ItemUse）/ string.Empty（Fatigue）
    string         SourceInstanceId, // CardInstanceId；无实例来源 = string.Empty
    string         SourceCardId,     // CardPlay / AbilityActivation / AbilityTrigger 有卡牌来源时非空；ItemUse / Fatigue = string.Empty
...
- **用道具自成一类 `ItemUse`，同样不与上述任何一类合流。** 判据与上一条逐字同款：因果树要读得出「他喝了一瓶药」与「他启动了阵法上的异能」之别，而两者的道念增量在快照上可以完全相同。条目取值：`EntryId = stackEntryId` · `CauseEntryId = string.Empty`（主动动作是因果树的根）· `SourceId = itemId` · `SourceInstanceId = string.Empty`（道具不是 `CardInstance`）· `Side` = 使用方 · `FizzledSlots` 照常。**敌人用道具同样广播本条**（它没有 `ActionResult`，战报是它唯一的可观测面）。

- **`SourceCardId` 与 `StackEntryView.SourceCardId` 逐字同构，理由同款。** 另外两个来源格都点不出「那张牌是哪张」：`SourceId` 在 `AbilityTrigger` 时是 `abilityId`，而 `AbilityData` 是独立可复用资源、由多个 `CardData` / `PowerData` 共同引用 ⇒ `abilityId → CardData` 的反查在设计上就是多义的；`SourceInstanceId` 的解析通道（对侧实例表）不在视图里。而**栈条目在结算完即消失、战报保留本场全部条目** ⇒ 缺这一格，战报展开态就永久失去「刚才触发的那张埋伏是哪张」，而「第 3 回合我的道念为什么少了 3」正是战报存在的理由。填法：`CardPlay` / `AbilityActivation` / `AbilityTrigger` 有卡牌来源时非空，`ItemUse` / `Fatigue` 为 `string.Empty`。**不由呈现层在栈条目消失前自行缓存**——那是同一事实的第二份持有，与「三个消费者读同一条流，各自去别处捞迟早不一致」相抵。本流不落存档 ⇒ **零 schema 影响**；负载仍只带 `Id` 与值类型，多的是一个 `string` 引用、无分配对象，热路径纪律不破。
...
Source: `handoffs/2026-09-09d-combat-rarity-and-reward-scale.md` · `handoffs/2026-09-09e-lifespan-item-supply-guardrail.md` · `handoffs/2026-09-09f-event-reward-and-hidden-stat-orchestration.md` · `handoffs/2026-08-30-affinity-and-technique-attributes.md` · `handoffs/2026-07-27b-service-api-contracts.md` · `handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md` · `handoffs/2026-08-06d-combat-open-questions-mass-closure.md` · `handoffs/2026-08-22-finale-failure-is-death.md` · `handoffs/2026-08-22-encounter-tighten-fields.md` · `handoffs/2026-08-25-numeric-philosophy-and-balance-anchors.md` · `handoffs/2026-08-25-combat-presentation-and-action-result.md` · `handoffs/2026-08-26c-enemy-ai-strategy-shape.md` · `handoffs/2026-08-26d-activate-ability-contract.md` · `handoffs/2026-08-28-item-use-effect-face-and-carrier-kind.md` · `handoffs/2026-08-30-stack-entry-kind-for-item-use.md` · `handoffs/2026-09-03-combat-snapshot-facedown.md` · `handoffs/2026-09-12-encounter-tighten-bounds.md`
...
life-cycle-service.AdvanceEventAsync(eventOption, mode, ct)
   ...
   │           └─▶ CombatResult（Outcome + 双方道念 + RemainingLifeSpan + Spoils:ProfileChangeSpec）
   └─ 【eventEnd 阶段】Spoils + lifeSpanCost + 隐藏属性推拉 → **一次** TryApply → 一个存档点
```

## 决策(-> ADR)

- **战斗模型 = mana + 道念；胜负 = 道念高者胜；失败按道念差扣寿元** → `decisions/ADR-0018-momentum-scoring-model.md`（Accepted）；规则骨架见 `systems/scoring.md`、`systems/adventure-event/combat/`、`systems/character-profile/life-span.md`、`mana.md`。
- **战斗定长，回合数为遭遇参数 `EncounterSpec.TurnLimit`（`Practice` 8 / `Standard` 10 / `Finale` 12，双方合计）；起始道念 = `baseMomentum`；道念可互削、下限 0**。
...
- **呈现事件流统一为一条广播 `CombatFeedEntry`**（卡牌结算 / 启动式异能 / 触发式异能 / 用道具 / 疲劳 / fizzle 六类共用，fizzle 是条目上的 `FizzledSlots` 一格而非独立类别）；**条目自带 `EntryId` / `CauseEntryId` 构成因果树**、存结构化数据不存格式化字符串；**不落存档，明写接受退出重进丢失**。
...
- **用道具在三个枚举上对齐为一等来源**：`StackEntryKind` 增 `UsedItem`（不复用 `ActivatedAbility`）、`CombatFeedKind` 增 `ItemUse`（不与启动 / 触发合流）；**栈条目新增 `itemId` 一格**（与 `kind == UsedItem` 互为双向不变式，三个来源格对道具恒空）；读档校验 ② 的强解析清单扩入栈条目的 `itemId`（`CombatItemSave.ItemId` 仍走 ⑥）；`UseItem` 的 mana 扣费与 `InsufficientMana` 与 `ActivateAbility` 同款；**「使用道具时」不开为触发时点**——时点随广播点一同增长，当前无内容需要它。

Source: `handoffs/2026-08-01b-abstraction-levels-combat-numbers-codex-family-and-monetization.md` · `handoffs/2026-08-02-momentum-conversion-reward-structure-and-mtg-stack.md` · `handoffs/2026-08-02b-stack-without-interaction-and-three-step-turn.md` · `handoffs/2026-08-03-battlefield-stack-hand-limit-and-power-item-naming.md` · `handoffs/2026-08-04b-mtg-loanwords-card-types-and-intent-snapshot.md` · `handoffs/2026-08-06d-combat-open-questions-mass-closure.md` · `handoffs/2026-08-11c-combat-turn-flow-fatigue-and-card-type-reduction.md` · `handoffs/2026-08-15d-intent-removal-lifespan-cost-visibility-and-design-audit.md` · `handoffs/2026-08-16-design-audit-adjudication-and-hand-limit.md` · `handoffs/2026-08-22-card-counters-api-and-key-space.md` · `handoffs/2026-08-25-combat-presentation-and-action-result.md` · `handoffs/2026-08-26c-enemy-ai-strategy-shape.md` · `handoffs/2026-08-26d-activate-ability-contract.md` · `handoffs/2026-08-30-stack-entry-kind-for-item-use.md` · `handoffs/2026-09-03-combat-snapshot-facedown.md`
