...
- **返回 `AdvanceResult` 而非 `void`。** 编排顶点需要一个可判定的返回值——**推进后的 `StatusAfter`** 尤其关键：`selectCost` 支付后可能直接判负，编排顶点据此转入失败流程而非事件屏。**没有「付不起 → 拒绝，回到呈现步」这条回路**，故 `FailedAt = CostRejected` / `MissingElement` 在事件推进路径上不产生（是否整体删除见待决问题）。
...
- **`eventEnd` 阶段把 `CombatResult.Spoils`（一份 `ProfileChangeSpec`）连同 `lifeSpanCost`、**战斗失败时按道念差扣减的 lifeTotal**、等级产出与隐藏属性推拉合并为一次 `TryApply`**，从而「一个事件 = 一次事务 = 一个存档点」。**分工：计算归 combat-service、施加归本服务**——奖励厚度、失败侧 lifeTotal 扣减（道念差 **1:1**）与可选奖励的玩家选择**全部在战斗流程内完成**，本服务收到的 `Spoils` 已是最终 spec，不重算。
...
- **`AdvanceResult` 的失败面还剩什么。** 事件推进不因「付不起」被拒绝，`FailedAt = CostRejected` 与 `MissingElement` 因此在这条路径上不产生；是否整体删除取决于别处（Exchange 商店购买等）是否仍需「余额不足即拒」。**连带：`TryApply` 施加负值时各资源的钳制规则仍未逐项定**（哪些截断到 0、哪些可为负、哪些的耗尽构成终态）——**隐藏属性那一半已答定**：道心 / 煞气施加后截断到 `[0, 100]`、**不构成终态**（寿元归 0 与 `lifeTotal` 归 0 构成终态）。→ `profile-service.md`、`systems/adventure-event/common-properties.md`。
...
- **隐藏属性细节：** 属性隐藏、`faith` = 道心、寿元按 `lifeSpanCost` 扣减 / 归 0 → defeated 均已定案；**取值域、档位表、阈值与回滞、跨档叙事形态已定案**（归 `systems/services/plot-manager.md`）。仍待定：**隐藏属性完整清单**（道心 / 煞气 / 寿元之外是否还有第四项）、**增减触发**（哪些 AdventureEvent 推拉、各推哪一档）、是否有非境界突破的寿元增长途径、AdventurePlot 树的数据编码。
