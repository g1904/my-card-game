...
- **账号级统计计数的字段形态与范围（08-06b 新增）。** 落成什么（`PlayerStatistics` 类？直接挂 `PlayerProfile` 的三个字段？）；除篇章重试外首批还统计什么（总轮回数 / 总 defeated 数 / 各 `DefeatReason` 计数）；它是否参与云端同步的权威性判定（**统计计数被篡改无玩法后果**，或许可走宽松口径）。→ `systems/player-profile/`、`systems/services/sync-service.md`。Source: `handoffs/2026-08-06b-asymmetric-ch1-band-consented-power-loss-and-chapter-retry-shape.md`。
