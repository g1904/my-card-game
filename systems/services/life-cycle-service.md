...
- **寿元是角色唯一的资源命线，归 0 → defeated。** **寿元 / lifeSpan** 既是寿命预算也是失败惩罚的承受量（炼气起始 100、抵达筑基 +100、抵达金丹 +300、抵达元婴 +500——元婴为游戏终点，该增量无可消耗预算，仅作最后一次数值更新并存档），**明文常驻、恒精确展示**（见 `ux/screen-flow.md`）。**它有两个扣减来源**：① 每完成一个 AdventureEvent，life-cycle-service 按该事件的 `lifeSpanCost` 扣减（内容侧为正数量值，物化时已取负）；② 战斗 / 修炼失败时按道念差 × `lossPerMomentum` 扣减（由 combat-service 算出、随 `Spoils` 交来）。递减到 **0** 即触发「大限将至」，角色置 `status = defeated`。语义权威见 `systems/character-profile/life-span.md`。`lifeSpanCost` 是 AdventureEvent 的共有字段（见 `systems/adventure-event/common-properties.md`），其分档是**控制篇章时长的主旋钮**（见 `systems/balance.md`）。
...
- **终结原因恰三种：主动弃置 / 寿元耗尽 / 渡劫失败**——前两种是资源触底、由资源表驱动判定，末一种是篇章闸门（`Finale` 档失败即终结，见 `systems/adventure-event/combat/_index.md`）。**`DefeatReason` 里没有「输掉一场普通战斗」这一项**——输一场 `Practice` / `Standard` 战斗本身不终结角色，扣的是寿元，故对应项是 **`LifeSpanExhausted`**。**恢复途径 = AdventureEvent 的 outcome / reward 与道具**，与等级 / `manaLimit` 同走 `ProfileChangeSpec` → `TryApply` 链路。
...
- **隐藏属性细节：** 属性隐藏、`faith` = 道心、取值域、档位表、阈值与回滞、跨档叙事形态的权威均在 `systems/services/plot-manager.md`。仍待定：**隐藏属性完整清单**（道心 / 煞气之外是否还有第三项）、**增减触发**（哪些 AdventureEvent 推拉、各推哪一档）、AdventurePlot 树的数据编码。**寿元的回复通道已定案**（存在，只走 outcome 侧；载体与平衡护栏见 `systems/adventure-event/common-properties.md`）。
