...
- **等级成长 = 事件产出经验值。** 境界内等级由 **AdventureEvent 的 reward 给予**，但**给的是经验值而非等级本身**：
  ...
  - **带经验的产出点约占事件总数 ≈55%（初值；ch1 参考构成实算 54.3%）。** 全覆盖会让经验变成「时间的自动函数」、事件选择在成长维度上失去差异；覆盖率过低（< 50%）则玩家为了升级只挑带经验的事件，压扁事件池多样性。**≈55% 贴着下限取**——它是「参考构成自洽 · **扣掉 N 次典型失败后的**供给 / 需求比落 1.15–1.20 · 覆盖率不破 50% 下限」三者能同时成立的取值（对账见 `systems/balance.md`）：过半的路都在前进，而选得好前进得快。
  ...
  - **供给 / 需求 ≈ 1.15–1.20，且这条带只管辖「扣掉 N 次典型失败后的比」`(G_c − N × X_c) / D_c`；满级后经验直接丢弃**（不结转、不开兑换通道，与「进阶即归位初期」同向）。**全胜路线的供给比是同一组输入的导出量，这条带不管辖它**——带的两条边界（更高则玩家提前满级、后半章奖励变废纸；更低则跳过几个事件就卡在境界末级进不了 Finale）描述的都是实际游玩路线的失衡，而全胜是上界情形、不是典型路线；而扣 N 后的比同时是表侧核对清单里的一条机械可校验断言，比一句书写口径更承重。**代价明写并被接受：** 带管辖扣 N 后的比之后，全胜路线的经验冗余高于带的上沿，熟练玩家在篇章后半段会更早看到经验奖励变废纸（满级后经验直接丢弃）。**供给账 `G_c` 因此必须计入胜侧道念差加成**（`experiencePoint` 也吃 `rewardPerMomentum` 那条线性加成），两个口径的取值与逐章对账见 `systems/balance.md`。**阈值曲线是事件数的从属量**：目标时长或事件定价一改，事件数随之变，曲线必须重算——ch1 的当前对账（35 事件 / 阈值合计 79）见 `systems/balance.md`。**卡级的实际后果 = 寿元耗尽而等级未满 → `defeated`**，这是**有意保留的失败面**。
...
- **选择界面 = 横向滑动选择区。** eventOptions 通过一个**可横向滑动的选择区**（horizontal scrolling area）呈现，玩家滑动以选中要继续的目标 AdventureEvent。契合月圆之夜风格的「事件菜单」形态，且贴合竖屏触控。**呈现与导航手感的权威在 `ux/screen-flow.md`**（等宽 carousel · 卡宽恒定按常态 3 项同屏标定 · 1–5 的自适应规则 · 焦点制进入手势）——本处只定进程形态，不复述呈现形态。
- **AdventurePlot 调制 eventOptions（方向）。** 隐藏抽象 **AdventurePlot（隐藏剧情线）** 是一棵分支可能性树，在背景中**调制 future-event-service 产出的 eventOptions**。隐藏属性（道心 / 煞气）达阈值时驱动对应剧情线，改写后续可选事件；某些节点可像 DnD 那样让玩家选择分支。详见 `systems/services/plot-manager.md`。
