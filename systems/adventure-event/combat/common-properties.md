...
- **战斗资源引用。** 战斗读取 `CharacterProfile.Status` 的 `currentMana / manaLimit`（无曲线；**每回合开始 mana 恢复至 `manaLimit`**，上限由事件 cost / reward 推拉），并维护**双方的道念**作为胜负标尺。寿元 `lifeSpan`（**单值，无上限字段**）**在战斗过程中不被读写**，只在收口时刻按道念差 × `lossPerMomentum` 扣减（见 `systems/scoring.md`、`systems/character-profile/life-span.md`）。战斗打满 `EncounterSpec.TurnLimit`（`Practice` 8 / `Standard` 10 / `Finale` 12，双方各占一半）后比道念定胜负。
