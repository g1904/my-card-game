...
- **战斗资源引用。** 战斗读取 `CharacterProfile.Status` 的 `currentMana / manaLimit`（无曲线；**每回合开始 mana 恢复至 `manaLimit`**，上限由事件 cost / reward 推拉），并维护**双方的道念**作为胜负标尺。`lifeTotal`（**单值，无上限字段**）**在战斗过程中不被读写**，只在结算时按道念差扣减（见 `systems/scoring.md`、`systems/character-profile/life-total.md`）。战斗**固定 10 个回合**（双方各 5）后比道念定胜负。
...
Source: `handoffs/2026-07-23-adventure-plot-hidden-stats-and-clarifications.md` · `handoffs/2026-07-30b-combat-level-intent-and-decision-point-saves.md` · `handoffs/2026-08-01-momentum-scoring-lifespan-tuning-and-failure-payoff.md` · `handoffs/2026-08-06d-combat-open-questions-mass-closure.md` · `handoffs/2026-08-15c-event-type-collapse-and-batch-shape.md`
