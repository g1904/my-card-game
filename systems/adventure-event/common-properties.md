...
- **`HiddenStatGrants` 不需要一条「不得写寿元」的校验。** `HiddenStat` 是两成员枚举 `{ Faith, Bloodlust }`，`(LifeSpan, …)` 在类型层面就写不出来——寿元不是隐藏属性，它走资源 element 路径。**校验 6（Travel 条目不得回寿）仍然必需**：它覆盖的是 `OutcomeRule` 两侧的 `ResourceKey == LifeSpan`，那条路径照常可写。
