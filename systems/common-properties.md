...
- **合法取值域按 `(CarrierKind, Scope)` 分域。** `(CarrierKind, Scope)` 是全库既有的分类键（置换同池判据即它全同），四类 = 该二元组的四个取值：
  ...
  - **`ExchangeSell` / `ExchangeBarter` 本身不构成结构改动**（字段形状不变，仍是一个整数 code，仅值域扩大——与下方那条通则一致），故它们不单独占 `schemaVersion` 登记表的一行；随行的 `EventOption` 物化字段属 `schemaVersion` 1，登记见 `systems/services/profile-schema-versions.md`。`PackSell` 一个字节也不进存档，与 schema 无关。
