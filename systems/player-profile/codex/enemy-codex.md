...
- **词条文案挂在 `EnemyData` 上，存档只记「已遭遇」。** 与「展示文案留在 `Resource` 上、存档态只带 `Id` + 可变状态」一致——**图鉴的存档负担因此接近一个 id 集合**，先前列出的「存档体积」顾虑基本消解。Source: 同上。
...
- **关键卡牌列 3 张，由 `EnemyData.KeyCardIds` 显式标注，不做自动挑选（已定案）。** 样本卡组 15 张，3 张 ≈ 玩家能记住的上限且足够勾勒路数（2 张不足以表达「路数」，5 张变成背卡表）。
  ...
  - **物化改写必须保留这 3 张**（否则图鉴与玩家实际遭遇对不上，而图鉴在意图黑箱档位下是唯一信息来源）→ 违反则 `PushWarning` + 该次改写回退；**`OverridesDeck == true` 的定制卡组条目显式豁免**。见 `systems/enemies/`。
  Source: 同上。
- **实例信息分离为「静态正文 + 动态页眉」（已定案）。** **词条正文绝不写等级**——词条挂模板、跨轮回持久，写入实例信息会污染静态知识（且同一模板会以 5 个不同等级出场）。**动态页眉只在战斗内查阅时出现**，元进程界面查阅时不显示（那里没有「本次」）。
  ...
  Source: 同上。
- **战斗内可查阅，但受限（已定案）。** 入口 = **点按敌人立绘 → 半屏 bottom sheet**（与「随身」抽屉同一套控件语言）。
...
- **五项写作规格（长度 / 口径 / 不含阿拉伯数字）；关键卡牌列 3 张由 `EnemyData.KeyCardIds` 显式标注；实例信息 = 静态正文 + 动态页眉；战斗内可读全部五项（结算 / 选目标态禁用入口）** —— 已定案。Source: `handoffs/2026-08-06d-combat-open-questions-mass-closure.md`。
