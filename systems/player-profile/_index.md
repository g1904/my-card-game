...
- **PlayerProfile 的完整字段表。** 本表**只有形态列**（字段 / 类型 / 层 / 写入通道 / 权威）——语义、取值域与读档校验一律留在权威列所指的文档里。**层** = 规则字段层 / 统计计数层（判据见下方分层通则）；**写入通道** = 该字段经 `ProfileChangeSpec` 的哪一列写入。`PlayerPower` / `PlayerItem` / `Achievement` / Codex 是**独立于任何单次轮回**的账号级解锁、成就与收集。
  ...
  | 14 | `entitlement` | `PlayerEntitlement`（2 字段） | 规则（透明段） | `Elements`（客户端置值 `BundleRedeemedOrdinal`）；`BundleGrantOrdinal` 由后端写、经 pull 下行，无客户端通道 | 本文档 · `systems/monetization.md` |
...
- **付费凭证 = `PlayerProfile` 上的具名小类 `PlayerEntitlement`，规则字段层，类内只放付费凭证本身与其兑现水位，不放任何派生量（承重）。** premium bundle 的持有状态改写篇章重试上限（无限 / 9 / 3）并授予随机 PlayerPower / PlayerItem，故它是 PlayerProfile 上的一项账号级状态：
  ...
  - **读档校验**：`BundleGrantOrdinal` `< 0` → `GD.PushWarning` + 钳制到 `0`；`BundleRedeemedOrdinal` `< 0` → 钳制到 `0`、`> BundleGrantOrdinal` → **钳制到 `BundleGrantOrdinal`**（判定为「无待兑现」），两向皆 `GD.PushWarning`。两者**都不由购买历史重建**（与三个首胜布尔同口径——它们是权威）。**钳制方向偏向不重复发放**：坏档下最坏是少发一次，而少发有后端对账信号可查，多发则是不可回收的发放侧漏洞。**不把 `BundleGrantOrdinal` 反过来抬高**——那是客户端改写一个只由后端写入的字段。
