...
- **标识符恒单数。** 元素类型 `Achievement`、集合字段 `achievement`，与 `PlayerPower` / `PlayerItem` / `CharacterItem` 同族——**类型名恒为单数，复数只属于集合字段名**。成就没有「储物袋」那样的已定名容器概念可供字段借名，故字段退回库内既有的单数风格（`pastEvent` / `disabledAbility`）。若日后全库统一把集合字段改为复数风格，本字段随那次统一一并改，不单独例外。**这是纯标识符收口，分组结构与两档奖励语义零改动。**
