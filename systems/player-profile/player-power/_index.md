...
- **道统残卷 / `PlayerPowerFragment` = 焊在 Finale 上的 PlayerPower 掉落概率（元进程的失败侧产出 · 承重）。** 失败不是零推进：
  ...
  - **掷骰走账号级 RNG，与 `CycleSeed` 完全解耦：** `rng = AccountRng.For(AccountStream.PowerFragment, FinaleWinOrdinal)`，`roll = rng.Randi() mod 10000`（万分比精度），命中 ⟺ `roll < 生效概率`。**具名域 `AccountStream` 是三参数派生的一部分**（不可退回两参数 `Hash64(AccountSeed, FinaleWinOrdinal)`）——礼包也走账号级掷骰后，两条渠道的序号会撞出同一序列；**命中判定语义不变，只是加了域**。掷中后的抽取**复用同一个 rng 实例连续抽**，故整次结算由 `(PowerFragment, FinaleWinOrdinal)` 完全确定。**绝不走 `SeedManager` 的四条子流**——它们全由 `Hash64(CycleSeed, streamName)` 派生，而篇章重试会生成全新 `CycleSeed`，挂上去等于让玩家靠重试换一次掷骰结果。**`FinaleWinOrdinal` 同时是幂等键**（同一序号重复结算得同一结果，退出重进 / push 重放都不改变掉落）。**对轮回可复现性零影响**——不派生自 `CycleSeed`、不消耗任何子流 `State`，故「残卷与 seed 公平性的关系」的答案是**两者不相交**。**执行方 = 客户端掷骰、后端可复算**（`AccountSeed` 在后端、序号与命中结果随 profile 上行），防篡改不因客户端执行而丢失，且 **Finale 奖励结算不引入任何新的网络往返**。见 `systems/common-properties.md`。
