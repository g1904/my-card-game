...
- **失去能力的频次预算：持久三支合计 ≈ 1 次 / 完整轮回，且不限于战斗。** 「稀缺性归内容侧纪律」这条量化口径的**承重表述取每完整轮回的期望次数，不取百分比**——百分比口径会被分母漂移静默重定价，而内容编排关心的是「玩家一局撞上几次」；百分比只作括号里的换算副本（**≈0.88%** 的全部 AdventureEvent（五类，含 Travel），分母 ≈114），分母变动时只重算括号、目标值本身不动。**分子 = 三支持久支**：法则置换型事件 · 法则禁用型事件 · **轮回级能力损失**（神通置换 / 禁用与法宝一族的置换 / 禁用同住这一支，挤进同一份预算、不另立一套，见 `../../character-profile/power/_index.md` 与 `../../character-profile/item/_index.md`）。**概率控制归内容侧**（与「稀缺性归内容侧纪律、代码只留 `PushWarning`」一致）。
  ...
  - **四支目标频次（次 / 完整轮回 · 待实测校准的初值）：**
    ...
    | **轮回级能力损失** | **≈0.67** | ≈0.59% | 上层口径内 |
    | &nbsp;&nbsp;├ 神通置换 + 禁用（置换 : 禁用 ≈ 2 : 1） | ≈0.32（置换 ≈0.21 / 禁用 ≈0.11） | ≈0.28% | 同上 |
    | &nbsp;&nbsp;└ 法宝一族（置换 + 法宝 / 古宝禁用） | ≈0.35（置换 ≈0.28 / 法宝禁用 ≈0.06 / 古宝禁用 ≈0.01） | ≈0.31% | 同上 |
    | 法则置换型 | ≈0.20 | ≈0.18% | 上层口径内 |
    | 法则禁用型 | ≈0.13 | ≈0.11% | 上层口径内 |
    | **上层口径 = 持久支合计** | **≈1.0** | **≈0.88%** | **承重表述** |
    | 四支合计（导出量 · 非闸门） | ≈2.11 | —— | 只作叙事密度 sanity check |

    **枚举仍是四支：神通与法宝一族同属「轮回级能力损失」这一支**，内部分列而不另开第五行。两者是同一类损失——置换同为轮回级 build 损失且同写 Profile，禁用同为「不影响持有、轮回结束即恢复」；按口径归属切表时它们落在同一格，另开一行会让读者看到的就是五支。

    持久支之间按「持久度 × 是否经玩家同意」两条轴分配：频次与「持久度 × 无同意」反相关——轮回级的可以最常见（神通随轮回清理、`ThisChapter` 档篇章边界自动恢复），无同意的必须最稀有。**法宝置换是全表频次最高的一格**：法宝在两条轴上都是最轻的一端（轮回级 + 有同意，储物袋不设容量上限、随售是常态弃置途径）。**法则禁用是唯一取低值的一格**——禁用是无同意的施加、最伤「构筑投入」契约，让出的份额转给同等严重度但有同意的置换侧（置换是正向决策点）。全序核对：法宝置换 0.28 > 神通置换 0.21 > 法则置换 0.20 > 法则禁用 0.13 > 神通禁用 0.11 > 法宝禁用 0.06 > 古宝禁用 0.01。
    - **禁用侧有一处刻意的偏离：法宝禁用取 0.06 而非按两轴外推的 ≈0.13。** 严格外推它应比神通禁用更常见，但**法宝禁用的单次体感是全表最轻的**（储物袋里仍在、`Charges` 分毫不动、只是本轮回不可启动），撑不起一条 `Rare` 档事件的戏剧分量；且它与置换共享同一个 0.35 的池子，份额给禁用等于削掉那格真正兑现「最轻的一端应最常见」的置换。
    - **古宝禁用 0.01 是给日后编排古宝预留的一格**：商店逐族库存深度记 `PlayerItem` 首批不编排 ⇒ 该格在首批内容上实际为 0，不要求为它编排条目。它与法宝禁用的 ≈ 6 : 1 兑现「禁用古宝的事件应比禁用法宝显著更稀有」；首批若改为编排古宝条目，这条比例须复核。
    - **法则禁用 0.13 与神通禁用 0.11 落在「每 7~8 个轮回一次」的量级，这不构成可见性问题。** 本作是 roguelike，玩家会反复开轮回——**小概率事件在深度游玩下必然发生**，这两格照样会被遇到、被记住。**可见性不由单轮回概率判定**，故「每 N 轮回一次太薄」不是一条能用来否决取值的判据（`decisions/ADR-0161-ability-loss-budget-absolute-frequency.md` 的备选段已按此改写）。上层总额 ≈1.0 是被明写的承重表述、不动，各格本就自陈为待实测校准的初值；变的只是这一次失去能力由谁贡献——由最轻的法宝一族顶上，方向与两条轴同向。

    频次的旋钮全落内容侧、零字段零校验零状态位：带 `AbilityChangeSlots` 的 `AdventureEventData` 条目数 + 各条目的 `SelectionWeight` 档（`Rare` 已是最低档 ⇒ 主旋钮实为条目数，典型每支 1–2 条 `Rare` 档条目；法宝置换 ≈0.28 在分母 ≈114 下折合 1 条 `Rare` 档条目，法宝禁用为 1 条更低权重条目或与置换条目共用载体，古宝禁用首批 0 条）+ 既有 arc 系数（`systems/services/future-event-service.md` 管线 ⑦）；`IgnoresProtection` 的旋钮是带该效果的 boss 档载体条目数与出现权重。
  - **法宝一族的失去事件同样在上层 ≈1.0 的分子内，份额 ≈0.35。** **法宝（`CharacterItem`）置换与法宝 / 古宝禁用一并计入上层 ≈1.0 的分子**，落在「轮回级能力损失」那一支内（逐格取值见上表）。判据是自持口径的第一条——`IgnoresProtection` 之所以能自持战斗分母，正因为**它不写 Profile**；法宝置换写 `CharacterProfile`，且法宝本就是上层口径所保护的「构筑投入」本身（与 deck、神通并列），故它触碰的恰是上层口径保护的那条心理契约。同层的神通置换（同为轮回级 build 损失、同写 Profile）本就在上层合计内，法宝一族排在合计之外需要一条不存在的新判据。
    - **给出份额是一次对分子的重新配平，不是填空。** 上层合计 ≈1.0 被两个 ADR 明写为承重表述、**不动**；法宝一族的份额只能从既有三支里割，故其余三支**等比压缩 ×0.65**（等比 ⇒ 全序关系与神通内部 2 : 1 逐格自动保住，改的是一个数不是形状）。窗口很窄：份额低于 ≈0.28 时法宝置换压不过法则置换，「最轻的一端应最常见」只剩算术上的成立；高于 ≈0.35 时法则禁用被压得过薄。取上端 0.35，法宝置换 0.28 对神通置换 0.21 有 1.3× 的可辨识梯度。
    - **玩家体感的位移如实写下：** 法宝置换从「一个轮回撞上两次」变成「约每 3~4 个轮回撞上一次」。法宝的日常周转（24 件供给、储物袋常态弃置）由**随售与售出**承载——那两条是储物袋的常态弃置途径，不写「失去能力」这本账。
  - **推论 ④：量级坐实。** 法则置换 ≈0.20 / 轮回、法则禁用 ≈0.13 / 轮回——两支各自落在**一个篇章遇上一次或更少**的量级，两支合计每三个轮回一次。「我的法则会不会被拆」因此是跨篇章尺度的稀有事件，与既定的「内容级稀缺保证而非类型级绝对保证」量级吻合。
