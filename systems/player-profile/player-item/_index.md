...
- **古宝恒不可售出。** 售出面仅对轮回级的法宝 `CharacterItem` 开放，且这是一条代码级常量判据，规则权威同上。**两条售出通道**（Exchange 商店内售出 · 储物袋随售）复用同一条常量，故古宝在两条通道上均不可售，唯一退出通道仍是置换。允许卖古宝 = 用账号级资产兑换轮回级货币，会使「开局清仓换经济」成为最优解，并与「避免第二套账号级经济」的取向相抵。随售通道的权威见 `../../character-profile/item/_index.md`。
...
- **古宝在储物袋中呈现，但持久层仍是 PlayerProfile。** 储物袋是**跨两个持久层的呈现视图**：轮回级法宝（`CharacterProfile.magicPack`）与账号级古宝同时呈现，条目上带 `AbilityScope { Character, Player }` 标识来源。古宝**不落 `magicPack`**，储物袋的轮回级那一半随轮回清理时古宝不受影响。视图形态的权威见 `../../character-profile/item/_index.md` 与 `ux/screen-flow.md`。
- **账号级持有条目另有一处轮回外只读落点。** 储物袋只在轮回内存在，故持有的古宝在轮回之外另有一处只读查看落点，形态见 `ux/screen-flow.md`。依据：古宝是付费礼包的主要交付物，付费资产的状态对玩家须恒可见。

- **已获得的 PlayerItem 会进 PlayerItemCodex。** 图鉴族（见 `../codex/`）为 PlayerItem 单列一本，记录静态文案；与当前**持有**的 `List<PlayerItem>`（含剩余使用次数）是两回事。

- **古宝在战斗内以 `CardType.Item` 呈现，与法宝走同一条路径（承重）。** 战斗道具区**同时呈现两级**，条目上带 `AbilityScope { Character, Player }` 标识来源。规则与法宝完全相同（不洗进卡组、**在储物袋中呈现**、不受抽牌运制约、使用窗口 = 自己回合的行动阶段且栈为空时、可带 mana 费用、`UsableScene` 三档、使用效果落 `CombatUseEffects` / `OutOfCombatUseOutcome` 两格且不带异能、本场配额落 `MaxUsesPerCombat`——两格使用效果面与配额面的权威在 `../../character-profile/item/_index.md`），差别只在**持久层与次数**：
...
- **古宝可被「本轮回禁用」，也可被置换。** 与法则 / 神通 / 法宝完全对称，**含 `ThisCycle` 档**：
  ...
  - **不违反「付费内容不会被游戏销毁」**：禁用不销毁、不扣次数、轮回结束即恢复。对法则开放而对古宝不开放，反而会让内容侧多背一条「哪些层能用哪些档」的例外表。但它确实是对付费内容的一次可感知削弱，故补一条**内容侧纪律**：**禁用古宝的事件应比禁用法宝显著更稀有，且一并计入「失去能力」频次预算的分子**——该预算取「次 / 轮回」口径，上层合计 ≈1.0 次 / 完整轮回（评审清单级，不加代码硬规则；口径与四支分表见 `../player-power/_index.md`。战斗内 `IgnoresProtection` 自持 ≈3% 的战斗分母、不计入上层合计，与本条不同支）。
  - **置换只在同池内进行**（`(CarrierKind, Scope)` 全同 + 同 `Rarity` + 排除已持有），即 `PlayerItem ↔ PlayerItem`；置换所得继承被换出条目的 `SourceCode`。规则权威见 `../player-power/_index.md`。
...
- **PlayerItem 的内容目录与次数补充未设计。** **已定：战斗内形态 = `CardType.Item`、`Charges > 0` 为硬约束、次数即时写 PlayerProfile、战斗外的效果形态（`OutOfCombatUseOutcome`，权威在 `../../character-profile/item/_index.md`）**（见上）。**仍未设计**：道具种类目录、次数如何补充、可购价格 / 库存权重。
