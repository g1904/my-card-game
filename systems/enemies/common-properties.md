...
- **显示字符串与 `Id` 分离**（名称、图鉴四项词条），可改动或本地化而不破坏引用。
...
- **「关键卡必在样本卡组内」是一条加载期即可执行的机械检查**：层数在模板上逐条固定 ⇒ 展开唯一确定 ⇒ `KeyCardIds` 中任一张不在「展开产物 ∪ 散牌」里 → `PushError`。理由：该字段是 AI 的打法重心标注，指向一张这个敌人打不出来的牌即是一条永不生效的编排（消费点见 `_index.md`「敌人 AI」）。
- **「核心功法必在功法引用列表内」同款**：`CoreTechniqueIds` 任一门不在 `TechniqueRef` 列表里 → `PushError`；列表为空且词条已启用 → `PushError`。理由：它是图鉴词条②的数据源，而图鉴与玩家实际遭遇对不上会当场废掉**事前知识的主通道**。
...
- **`ChapterScope` 留空是机械不可发现的**（漏填与「有意三章通用」形状相同）。缓解手段是**纯结构判定**、不做文案扫描：`ChapterScope` 为空的条目在 `content/enemy/` 类型档案的评审清单里逐条过一遍，确认其图鉴词条三章都说得通。**不按境界词（筑基 / 金丹 / 元婴）做子串扫描**——图鉴四项词条的类型是 `LocalizedText`，子串规则在任何非中文语言下都失效，中文侧也会误命中。
