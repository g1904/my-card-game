...
**尚未建立任何服务文档。** 后端技术栈与托管形态未定（`open-questions/06-platform-stack.md`），协议契约也只成文了边界层与内容分发两份（`contracts/envelope.md` · `content-manifest.md`），各端点报文本体仍待写（`01-contracts.md`）——在这两者落定之前，服务的内部设计无从谈起。按「先有设计再建文件」的约定，此处不预先占位空壳。
