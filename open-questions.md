...
**技术栈与托管形态已落定**（C# / ASP.NET Core · 腾讯云托管容器 · PostgreSQL 单主 · Redis · KMS · CDN），
`systems/` 三份服务文档去其二、`operations/` 八份文档去其七——全库再无结构性前置。焦点因此下移到外部与排期：

1. **`02` 合规能力的上线分级** —— 唯一一条会改变「首版能不能上」的问题：实名 / 防沉迷 / 注销 / 导出哪些必须在首次上线前具备。
...
- **合规能力的上线分级（`02`）** —— 它决定首版的验收面有多大，且与备案 / 渠道过审排期耦合。
- **外部依赖的选型与资质（`06`）** —— 服务商、灾备、渠道验票凭据托管；**微信开放平台资质必须在首个玩家建号之前完成**。
- **成本模型** —— 它是一批数值的共同前置（实例规格、灾备副本数、备份保留期、CDN 成本），本库已把这些一律写成能力要求而非具体数字。
