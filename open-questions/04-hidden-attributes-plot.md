...
- **剧本树不分包的三项配套裁决 —— `[采纳推荐 — 待复核]`（08-22 新增）。** 主结论「**不按篇章分包**」已落 `systems/services/plot-manager.md`（承重理由是分包与「合并后全量强校验」的结构冲突——跨包引用是剧本层级模型本身的形状，非体积）。用户授权按推荐落笔，三项仍待正式复核：① 关闭为定案 + 留一道零成本复核闸（而非保留为「已倾向不分包、待第二阶段实测」）；② 体积护栏只在 `content/plot-arc/_index.md` / `content/plot-node/_index.md` 的条目台账记「条目数 + 字节总和」，不加加载期校验（两个类型档案尚未开张，随开张落地）；③ 「平台原生按需资源」只写一句方向性记录，且不往后端库 `contracts/content-manifest.md` 写承接。另：复核闸的两个门槛（占比阈值 / 绝对值上限）待第二阶段有实测数时再定——当前无依据，且在第一批真实剧本条目写完前也无从执行。→ `systems/services/plot-manager.md`。
