...
- **剧本内容的体积与分发粒度（08-11 新增）。** 三篇章完整剧本树的包体 / 下载量；是否按篇章分包、按进度增量下载（文件级事务已现成，**分包边界**未定）。本地化后这成为一笔真实成本——原云端方案的「按需请求」天然回避了它。**语言维度已于 08-13 排除在外**：多语言定为全语言内嵌同一 `.tres`、**不按语言分包**（双语封顶 ⇒ 体积上限固定 ×2，分包的唯一实质动机消失），故本条回归其原本形态——**剧本树该不该按篇章分包**，两者互不牵动。→ 同上。
