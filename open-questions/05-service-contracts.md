...
- **存档 `schemaVersion` 登记表的覆盖对账（08-30 登记的「bump 清单已漏三批」条目改写而来 · 主体已收口）。** 原条目的三件事全部已答结：清单已拆出为 `systems/services/profile-schema-versions.md`、四批中三批在 v1 行 #8 / #22 / #27（`Status` 删三格属形态纪律 ④「首发前删除的字段不进任何版本行」有意不登）、全库自称已改回链、后端两份契约已改指登记表。**残留已于 09-05 落笔收口**：v1 行 11 处覆盖空缺补为 #28–#33、#18 错归属订正、`achievement` 留占位、#12 与 `ADR-0127` 两处不可核对计数删除、形态纪律 ⑥「本表不写任何计数」立案、`ProfileShapeCheck` 补一条 `/sync-knowledge` 条目 ↔ 字段表行双向对账断言。**留在清单上的只有两件**：① `PlayerProfile.achievement` 的条目结构待 `Achievement` schema 答定后补入 v1 清单（前置在 `systems/services/profile-service.md` 的待决问题，本条不重复登记）；② 那条 `/sync-knowledge` 对账断言的实际生效跟随该技能下一次运行，无排期依赖。→ `systems/services/profile-schema-versions.md`。
