...
- **`#if DEBUG` 判据的实测确认（08-09 新增）。** 离线后端「Release 构建里根本不存在」这一主闸依赖「Godot 的 .NET 集成在 Release 导出配置下不定义 `DEBUG`」。`game-feature-branch/` 当前**尚无 `.csproj`**，无从验证；**首次生成 `.csproj` 后需实测一次**。若不成立，改用显式 `<DefineConstants>MYCARDGAME_OFFLINE_OK</DefineConstants>`，方案形态不变。→ `system-overview.md`。
  ...
  - **同批实测（09-02 新增）：平台凭据存储的三项能力事实。** ① Godot 4.7 有无内置安全存储 / 密钥库绑定（决定移动双端是否必须自建原生插件）；② Web 导出 `user://` 的具体持久化后端与持久化保证；③ Android Keystore 密钥失效（锁屏 / 生物识别凭据变更、换机恢复）的具体异常与触发面。三项均**只影响能力矩阵中带 `[待核实]` 标注的格子**，不改变触发条件与「不引入平台分支」结论，故不阻塞任何已落笔内容。→ `systems/services/account-service.md`。
- **存档 `schemaVersion` 登记表的覆盖对账（08-30 登记的「bump 清单已漏三批」条目改写而来 · 主体已收口）。** 原条目的三件事全部已答结：清单已拆出为 `systems/services/profile-schema-versions.md`、四批中三批在 v1 行 #8 / #22 / #27（`Status` 删三格属形态纪律 ④「首发前删除的字段不进任何版本行」有意不登）、全库自称已改回链、后端两份契约已改指登记表。**残留已于 09-05 落笔收口**：v1 行 11 处覆盖空缺补为 #28–#33、#18 错归属订正、`achievement` 留占位、#12 与 `ADR-0127` 两处不可核对计数删除、形态纪律 ⑥「本表不写任何计数」立案、`ProfileShapeCheck` 补一条 `/sync-knowledge` 条目 ↔ 字段表行双向对账断言。**留在清单上的只有两件**：① `PlayerProfile.achievement` 的条目结构待 `Achievement` schema 答定后补入 v1 清单（前置在 `systems/services/profile-service.md` 的待决问题，本条不重复登记；答定后仍属 `schemaVersion` 1、不产生新 bump，在此之前不得写入推测形状）；② **v1 的 golden 形状快照 `profile-shape-v1.json` 待建**——设计形态已完整（`ProfileShapeCheck` 节），卡的只是落地时点（`game-feature-branch/` 尚无 `.csproj`），宜与本片上一条的三项实测同批落地，不单独排期、不阻塞任何 derive。**（09-07 订正：此处此前登记的是「那条 `/sync-knowledge` 对账断言的实际生效」，而该断言在主题文档中已是既定纪律、不在其待决小节内；主题文档实际留在待决的第二件是 golden 快照。）** → `systems/services/profile-schema-versions.md`。
  - **备注（指向索引）：** `../open-questions.md` 的 `## derive 就绪度` 小节内，`sync-service.md` 判定行的「⚠ derive 前置」、「落笔前的零决策修正 ①」与 derive 顺序第 7 步的「落笔前须先补齐本文件的 schema bump 清单」三处，讲的都是**已消失的前提**（清单已拆出、已补齐、两侧回链方向已修正）。该小节由 `/assess-derive-readiness` 独占写入，故本次不就地清理，**待下次全量评估随扫描一并刷新**；在那之前读到那三处的人以本条为准。
