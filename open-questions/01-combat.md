...
- **道念对比的视觉形态、回合进度与道念差的组合呈现。** 主视觉地位已定，形态未定；形态须容纳「双方头像与数值分居对比条两端」这一已定落位。**「道念差是否显式呈现」的支持论据弱于逐点对应**——负侧换算是「道念差 × `lossPerMomentum`」，三章系数各不相同（10 / 5 / 10）⇒ 任何一章都要先乘一个系数才折得出寿元代价，比逐点对应多一步心算。（「寿元是否常驻战斗屏」已答结：不常驻、结算面板如实展示，见 `../answer-logs/log-life-lifespan-merge.md`。）→ `ux/combat-ux.md`。
