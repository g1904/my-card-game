...
- **`RarityTier` 的分布与权重表（08-10c 新增）。** 五档已定名并挂上 `PowerData` / `ItemData` / `CardData`；各档在**奖励池与置换候选池中的权重**（按优势档 `Tier` 三档各一张表）、以及内容侧「每档应有多少条目」的编排口径未定。→ `systems/balance.md`、`systems/services/combat-service.md`。
- **`StatKey` 的完整成员清单（08-10c 新增 · 轻）。** 首批两项已定；随统计项增长的命名与登记方式、如何在书写上与 `CostKey` 明确分开未定。→ `systems/services/profile-service.md`。
- **一次合并收口的机会（非阻塞）。** `PlayerPower` 两条获取渠道的候选池与排重规则（`07-codex-monetization.md`，残卷 / 礼包抽哪一条）**与置换候选池是同一形状的问题**——`AllEnabled()` 全池 → 排除已有 → seeded 抽一条，差别只在残卷侧不限稀有度、走账号级 RNG。宜一次答定、共用同一段抽取伪码。→ `systems/monetization.md`。
...
- **候选里出现已持有功法怎么办（08-12f 新增 · 轻）。** 学新功法的三选一抽取若命中卡组中已有的功法：排除？还是折算为一次升阶？→ `systems/character-profile/deck/`。
- **卡组被弃空的内容侧态度（08-12f 新增）。** 弃置**不设限**（角色绑定的两门也能弃）⇒ 卡组可被弃空。规则层无需新规则（疲劳规则已表达后果，且「输是正常出口」是既定取向）；仍待定的是**内容侧是否回避**（例如只剩一门功法时不再提供弃置选项）。→ `systems/adventure-event/`、`systems/balance.md`。
- **功法 / 法宝三选一的 RNG 子流归属（08-12f 新增 · 轻）。** 复用既有 `reward` 子流，还是新开一条。→ `systems/common-properties.md`。
