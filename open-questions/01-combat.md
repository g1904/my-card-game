...
- **敌人各等级的道念产出缩放曲线（本次归集 · 此前未进清单 · ⚠ 带一处主题文档间的矛盾，待用户确认）。** 起始值已由 `baseMomentum` 给定；**产出**能力随等级 / 境界 / 篇章如何缩放，`systems/enemies/_index.md`、`systems/enemies/common-properties.md`、`systems/adventure-event/combat/_index.md` 三处均登记为「未定 → `systems/balance.md`」，而 `systems/balance.md` 的对应条目写的是「**敌人侧的产出结构亦已定**（无敌方专属缩放曲线，产出决定因素与玩家一致）」。**两种读法互斥**：若 balance 侧成立，敌人产出随「等级 → 功法层数 → 卡组」间接缩放、本条不存在；若三处 enemies / combat 侧成立，则还欠一条独立曲线。本技能不裁决，先如实登记。→ `systems/balance.md`、`systems/enemies/_index.md`、`systems/adventure-event/combat/_index.md`。
...
- **道念对比的视觉形态、回合进度与道念差的组合呈现。** 主视觉地位已定，形态未定；形态须容纳「双方头像与数值分居对比条两端」这一已定落位。**「道念差是否显式呈现」的支持论据弱于逐点对应**——负侧换算是「道念差 × `lossPerMomentum`」，逐章系数不同（**ch1 = 10 已锁定，ch2 / ch3 的候选值 5 / 10 尚未定案**，见 `systems/balance.md`）⇒ 任何一章都要先乘一个系数才折得出寿元代价，比逐点对应多一步心算。（「寿元是否常驻战斗屏」已答结：不常驻、结算面板如实展示，见 `../answer-logs/log-life-lifespan-merge.md`。）→ `ux/combat-ux.md`。
...
- **竖屏分区的整体排布（本作压力最大的一处 · 08-15d 加压说明）。** 需容下战场 / 栈 / 手牌 / 双方道念位 / 回合计数 / 战报，再加法则条、随身角标、埋伏标记；**08-25f 加压：新增玩家侧头像位**（战报因收起 / 展开共用同一格而净增 0 个区）；**己方战场区左右两侧同时挂法则条与随身角标会挤压战场可用宽度，须在实际排版时验证**。**体检登记：根子是把 MTG 的完整分区模型搬进手机竖屏**——尤其**栈在零交互下玩家做不了任何事、只能看**，它是否必须常驻（而非结算时的临时演出层）值得重估。意图区移除已释放约屏高 8%，但未触及根因。**已排期：单开一个专门 session 答疑（08-16 确认）**，不在逐次 handoff 中零敲碎打。**待容纳形态**：只读层（神通 / 法则条）的形状（角标 vs 常驻一排小图标）——**神通不设持有数量硬上限 ⇒ 只读层的条目数无上限，该形状必须能容纳 N 条** · 静态启动可供性标记的具体形状（`Power` 与阵法条目**共用同一套**，专场并案处理）· 随身抽屉内 `AbilityScope` 两级标识的视觉形态 · 己方面朝下条目（埋伏）在己方战场区是逐条渲染还是折叠成一格。→ `ux/combat-ux.md`。
  - **同专场的四项具体形态（08-16b 采集 · 08-30 并入本条）：道具区 / 神通法则条 / 埋伏标记 / 卡牌类型标识。** 落位与视觉语言已定方向（道具 = 角标 + 抽屉、`Power` = 低权重小图标条且长按查看详情、埋伏 = 只给计数、卡牌类型 = 卡框色 + 类型角标，**且仅适用于会以卡面出现的类型**）；**具体形态未定**——其中卡框色与类型角标在 full art 下是卡面唯一的非 `manaCost` 呈现元素，其权重与可辨性因此升格。它是本条竖屏分区压力的四个具体来源。→ `ux/combat-ux.md`。
...
- **敌人台词的槽位清单 `LineSlot` 的成员（本次归集 · 此前未进清单）。** 字段形态已给出（`EnemyData.Lines`），**成员清单待一次战斗 UX 专场**——台词在战斗屏的呈现落点尚无表述，在它答定之前 `Lines` 只能是空数组。`systems/enemies/common-properties.md` 的待决项已点名回链本分片，而本分片此前零承载。归入上一条的竖屏专场一并处理。→ `systems/enemies/common-properties.md`、`ux/combat-ux.md`。
