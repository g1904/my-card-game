...
- **图鉴的入口与浏览形态。** 六本图鉴在主菜单如何组织（一个「图鉴」入口下分六页？）、战斗内能否查阅（EnemyCodex 尤其相关）。→ `ux/screen-flow.md`、`ux/combat-ux.md`。
- **纯外观付费点做成什么（08-15b 新增 · 轻）。** 「是否真做」已答定——**架构预留、首批不做**（未来会做；落地时 = `PlayerEntitlement` 加一个具名字段 + 一次 schema bump，首批不新增字段、不新增屏）。仍待定：做成角色皮肤 / 卡背 / 界面主题的哪些、字段落成什么形状。通行证 / 赛季已明确「当前不做」。→ `systems/monetization.md`。
- **平台内购 SDK 的选型与封装层（本次归集 · 此前未进清单）。** 三渠道（Google Play Billing / App Store / 微信支付）**已纳入 MVP**（`vision/scope.md`），且它们是客户端**唯一必须引入第三方 SDK 的地方**——牵动 Godot 导出配置与各平台构建。选型、封装层形态与三渠道的收据差异归后端的支付渠道选型与一次专门的**客户端工程蓝图**，不在本库定稿；**购买段因此在 SDK 落定前无法落地**，兑现段不受影响（唤起失败一律回主菜单，无 Profile 变更、无痕迹）。→ `systems/monetization.md`、`backend-design-documents/`。
