...
- **付费角色系列的两侧承接已于 2026-09-12 同批落笔，两侧无遗留欠账。** 本库落解锁载体与购买流程（`CharacterData` 的 `SeriesId` / `Track` 两格与五条加载期校验 · `PlayerEntitlement.CharacterSeries` 与读档校验 · 取池过滤落 `life-cycle-service` · `schemaVersion = 2` · Store 屏三结果态 · 购买时限的三条语义），权威见 `handoffs/2026-09-12-premium-character-series-unlock.md`；对侧同批落 SKU 粒度与 `productId ↔ seriesId` 机械变换、验票写入与幂等语义、`profile-sync` 两张表各加一行（**后端写入字段封闭表成文以来的首次扩表**）、SKU 表的上架窗口字段与兼容矩阵 `schemaVersion = 2` 行，见 `backend-design-documents/handoffs/2026-09-12-premium-character-series-unlock.md`（**本库不复述其形态**）。
  > **两条须一并成立的相互依赖，落笔时已对齐、此处只作留痕：** ① 对侧对 `/entitlement/characterSeries` 取**有序逐元素**比对，其前提是后端恒在尾部追加且**本库原样回声、不重排 / 不去重 / 不归一化**（该纪律已写入 `systems/player-profile/_index.md`）；② 兼容矩阵**先加、客户端后发**——矩阵未加就发客户端，则新版客户端的第一次 push 即被 `sync.payload_schema_unsupported` 拒。
  > **条件分支：** 若付费系列改在首发**之前**落地，本库 `schemaVersion` 登记行并入 v1 清单、对侧矩阵行整行不发生；结构与该分支无关。
...
- **账号身份模型两侧同批落笔**（2026-08-16）：后端定契约本体（`backend-design-documents/contracts/auth.md` 的身份模型与绑定 / 改名端点），本库同批落 `account-service` 的四个新方法、`AccountInfo` 的三个新字段与绑定 UX。**两侧无遗留欠账**；当时余下的 `deviceId` 落点与 refresh token 持有形态是本库自己的待决问题（非跨边界承接项），**两条均已答定**——见 `../answer-logs/log-device-id-provisioning.md`（08-19）与 `../answer-logs/log-refresh-token-client-storage.md`（08-22），权威在 `systems/services/account-service.md`。移出记录见 `../answer-logs/log-account-identity-model.md`。
