...
- **球在对侧的三条（2026-08-22 同批产生，本库侧均已落笔）：** ① **回声校验通则的后端半** —— 客户端半已落 `systems/services/sync-service.md`，后端的受约束 path 清单与非整数路径比较口径待落笔，承接项在 `backend-design-documents/open-questions/01-contracts.md`。**⚠ 两份草稿都明写须成对采纳，本批只落了客户端半——成对采纳尚未完成。** ② **flags 回滚即前滚的对位条款** —— 客户端已采纳「增大即拉」并开始依赖它，承接项在 `backend-design-documents/contracts/content-manifest.md`。③ **静默续期绕过协议维度强更闸门的收口手段** —— 客户端已明确**不自收口**（自加周期判断会撞「设备时钟不可信」纪律），滑动续期上限 / 强制 re-signin 周期归对侧定，承接项在 `backend-design-documents/contracts/auth.md` §5。三条本库侧均无欠账，只等对侧落笔。
