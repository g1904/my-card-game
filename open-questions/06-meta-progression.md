...
- **`completed` 是否清理角色数据（09-05 新增）。** `systems/character-profile/_index.md` 与 `systems/services/life-cycle-service.md` 开头写「`defeated` 与 `completed` 数据都会在轮回结束时被清理」，而同一份 `life-cycle-service.md` 的「篇章继承：全部继承」与「篇章通关即在所达境界落存档点，可读档从该境界起始下一篇章」、以及 `program-overview.md` 阶段 5 的 `completed` 分支（只落存档点 + 解锁下一篇章可挑战角色，不写清理）要求它**不**被清理。两处措辞须择一收口。**不阻塞篇章结束屏**——该屏的摘要在 `TeardownCycle` 与任何角色数据处置之前组装，两种读法下均成立。→ `systems/services/life-cycle-service.md`、`systems/character-profile/_index.md`。
