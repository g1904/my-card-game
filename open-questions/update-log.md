...
## 2026-09-12 — 清单专职重整：**零移出**，修 15 处失效回链与三处失真（`/summarize-open-questions backend`，批量运行 `progress-sync` 波次 3）

一次专职归集整理，不引入新想法、不裁决任何问题。**本次无任何条目可移出，故不新建 answer log**，`answer-logs/_index.md` 台账零增行。`## derive 就绪度`（波次 2 刚整体重写）与 `## 下一阶段` 两节**一字未动**。

- **待答面（5 条，无增无减）**：`01` 一条（微信下单端点在上架窗口外的应答）· `06` 两条待实测取值（成本模型 · `riskEventBufferRows`）· `07` 两条相邻真空（告警接收人 / 值班形态 · 客服侧的账号查询入口）。另有**不属设计待答**的两类：`02` 一条外部事实挂账项 · `04` / `06` 各一节条件化核对项。`cross-boundary`「待承接」仍为空。
- **波次 1 的五份新 ADR（`ADR-0064` ~ `ADR-0068`）对待答面零影响**：五份固化的都是 09-12 已落笔的正文，**未答结任何一条待答**。尤其 `ADR-0068`（上架窗口只约束「能不能买」）在「后果」段**明写「本决策不预判它」**，指的正是 `01` 那条 —— 本次据此在 `01` 的条目末补一句，防止后续会话误以为该 ADR 已覆盖。
- **主题文档 ↔ 分片逐条对账**：全库 **7 处** `## Open questions`（`vision/scope.md` · `contracts/auth.md` · `compliance.md` · `envelope.md` · `profile-sync.md` · `purchase.md` · `operations/internal-tools.md`）逐一直读 —— `auth.md` / `compliance.md` / `profile-sync.md` 三处自陈「无待答项 / 无」；`purchase.md` 一条与 `01` 分片**逐字同题**；`internal-tools.md` 两条与 `07` 分片**逐字同题**；`scope.md` 把唯一相关项指回 `06`；`envelope.md` 唯一条目自陈属待落笔项，已由 `01` 抬头承接。**零漂移、零遗漏。** `handoffs/` 43 份**全为 `distilled`**（`raw` / `triaged` 零命中），`inbox/` 顶层为空 ⇒ **无未进清单的散落未决项**。
- **本次实际修的三处失真（均在待答清单自身，非主题文档）：**
  1. 索引「当前焦点」仍写「`01` 分片待答清零」并把焦点归为**三类** —— `01` 已于 09-12 新增一条，措辞与实际相反 ⇒ 改为**四类**，`01` 那条置首（它是全库唯一一条设计取向面的待答）。
  2. 同段仍写 `operations/` **九份**文档 —— 实为 **10 份**（`operations/_index.md` 台账 10 ↔ 10）⇒ 订正。同款失真在 `README.md:34` 亦有一份，**不在本技能写入面，只报告**。
  3. `04-content-delivery.md` 残留标签 **A7–A9** —— 该组编号已随内容迁入 `systems/content-delivery.md`（现为实现纪律 + F1–F8），全库按该标签检索零命中 ⇒ 改写为「三条服务端纪律」并补现落点与 `ADR-0047` 回链，同时写明旧编号已不存在。同源残留在 `operations/content-delivery-ops.md:4` 的 Source 行仍在，**属主题文档，只报告**。
- **15 处失效回链**：`01`（12 处）· `04`（2 处）· `06`（1 处）在分片内写 `` `answer-logs/log-*.md` ``，而分片自身位于 `open-questions/` 下 ⇒ 相对路径少一层，点开即 404 ⇒ 统一改为 `` `../answer-logs/…` ``（同文件内本已有若干处写对的，形态就此一致）。
- **台账对账**：`open-questions/` **7 份文件 ⇔ 索引分片导航表 7 行**；`answer-logs/` **36 份 log ⇔ `_index.md` 36 行**。均零悬空行、零孤儿文件。索引顶部说明块 **15 行 / 最长 87 字**，两项上限（≤15 行 / ≤200 字）均未触及，**无需下沉**。
- **一处工程提示**：本文件已达 ~136 KB，远超 `index-size-guard.ps1` 对 `update-log.md` 的 48 KB 阈值（该钩子只告警不拦截）。裁剪属维护动作，本次不自行执行。

## 2026-09-12 — 付费角色系列那批定案立档：新增 `ADR-0064` ~ `ADR-0068` 五份（`/write-adr backend`，批量运行 `progress-sync` 波次 1）

一次专职立档，不引入任何新设计、不裁决任何决策。**候选清单侧零变动**——`open-questions.md` 的 `## 下一阶段` 不含 ADR 候选条目，`decisions/_index.md` 的「ADR 候选」表已于 08-19 整节删除。候选来源是唯一一份晚于 `ADR-0063` 的 handoff（`2026-09-12-premium-character-series-unlock.md`，全文未自评 ADR 候选——与 `ADR-0054`~`0059`、`ADR-0063` 六份先例同形）。五条逐条按「台账绝不领先于事实」直读核对过主题文档才建档：**零条查无实据、零条与主题文档矛盾。**

- **`ADR-0064`** 付费角色系列取系列级 SKU，`seriesId` 由 `productId` 机械变换，后端零内容知识。依据：`contracts/purchase.md` §3a「SKU 类别与 `productId ↔ seriesId` 的机械变换」· `operations/purchase-ops.md` §1a。
- **`ADR-0065`** 验票应答按 `kind` 分形，请求侧一格不加（`seriesId` 不进请求 · `bundleGrantOrdinal` 条件化 · `GET receipt` 同款分形 · 不新开端点）。依据：`contracts/purchase.md` §3 · §4。
- **`ADR-0066`** `/entitlement/characterSeries` 进后端写入字段封闭表——**该表成文以来的第一次扩表**，两条「够格进表」判据逐条论证，护栏措辞与「受约束的顶层键恰有两个」一字不改。依据：`contracts/profile-sync.md` §5 护栏段 · §5c。
- **`ADR-0067`** 重复购买一个已拥有的角色系列：接受写入 + 风控事件，绝不拒绝、不自动退款。依据：`contracts/purchase.md` §3c · `operations/purchase-ops.md` §5。
- **`ADR-0068`** 角色系列的上架窗口只约束「能不能买」，不进 profile、不进任何封闭表。依据：`contracts/purchase.md` §3a「上架窗口」· `operations/purchase-ops.md` §1a。
- **日期一律取 2026-09-11**（用户对来源草稿逐项裁定的那天，见该 handoff 的 `## Clarifications`），不取建档日——同 `ADR-0063` 的先例。
- **识别为候选但按判据不建档的三项**：`schemaVersion = 2` 登进矩阵（`ADR-0024` 既有流程的一次应用）· 失败 `code` 零新增（`envelope.md` §6 既有判据的一次应用）· 幂等记录加三格（`kind` / `granted_series_id` / 存 `productId` 原值，属 `ADR-0045` 覆盖面下的存储形态细节）。三者均无独立可推翻的方向性内容。
- **台账**：`decisions/_index.md` 决策表新增 5 行（最新置顶，同日按编号降序）；对账 **68 行 ⇔ 68 份 ADR**，无孤儿文件、无悬空行，**本次无失真可修**。「已对后端构成约束的客户端决定」表一格未动。`open-questions.md` **一字未改**（`## 下一阶段` 无可移除条目；`## derive 就绪度` 属 `/assess-derive-readiness` 独占写入，本次五份新 ADR 的登记留待下一次全量评估）。
- **未处理的相邻漂移**（属主题文档，不在本技能写入面）：`ADR-0017` / `ADR-0025` 的回链仍是单向的（第六次记录）；`ADR-0061` ~ `ADR-0063` 的单向回链同样未补，本次五份新 ADR 在正文中亦无回引（`purchase.md` 的 `## 决策(-> ADR)` 段仍只列三条）。

## 2026-09-12 — 付费角色系列的后端承接落笔（`/batch-analyze-new-ideas` 五份批次的后端分片 · 跨边界移出 1 条 · `01` 新增 1 条 · 跨库成对）

一次批量运行（客户端四份 + 后端一份，run 目录 `.claude/batch-runs/2026-09-12-five-reviewed-drafts/`）。草稿的全部取向项已于 2026-09-11 批量评审裁定并写回草稿，**本次无 interview、无重开**。

- **移出 1 条 → `../answer-logs/log-premium-character-series-unlock.md`：** `cross-boundary.md`「待承接」第一条（客户端 `ADR-0255` 的三项义务）整条答结并落笔，两小问同批定案——**SKU 粒度 = 系列级且只有这一种**（单解 SKU 不存在，「付 N 个单解之价」是定价锚，不对应任何可购商品）· **封闭表两张都加**（后端写入字段封闭表与透明字段白名单正交、非二选一）。该条移出「待承接」、改写后落入「对账基线」区首条，**「待承接」随之为空**。
- **后端写入字段封闭表成文以来的首次扩表（4 行 → 5 行）**，用户在评审中明确批准。护栏的例外式措辞规则（「后端只读，除表内 N 项外」）与 §5c「受约束的顶层键恰有两个」**一字未改**——`entitlement` 本就是两个受约束顶层键之一，故顶层键集合不变。加行的正当性按「够格进表」两条判据逐条论证写入正文。
- **`envelope.md` 零改动：`code` / 端点 / `OpError` 全部零新增** ⇒ §6 台账与 §3 端点全集表均不加行，**P-1 / P-3 两条护栏不被触碰**，三条机检断言的双向映射零变化。§8 的连带刚性（向受约束顶层键内追加字段须两侧同批落笔）**首次真实行使**，条文不改。
- **承接客户端评审中新增的「上架窗口」机制**（限时销售窗口 · **不绝版** · 已购玩家永久可用）：落 `operations/purchase-ops.md` 的 SKU 表一格 + `verify` 侧对窗口外新购的拒绝（复用 `purchase.receipt_invalid`）。**窗口不进 profile、不进 `/entitlement/characterSeries`、不进任何封闭表**——它约束「能不能买」，不是「拥有什么」。
- **新增 1 条 → `01-contracts.md`（该分片由「清零」转为 1 条）：** 微信下单端点在上架窗口外的应答码。窗口校验现只落在 verify（付款**之后**），前移到 `order` 更合「把失败点挪到掏钱之前」，但没有语义合身的既有 `code`。**不阻塞首版**（微信渠道首版不开通），但它是本次「失败 `code` 零新增」结论的唯一潜在破口。
- **`schemaVersion = 2` 登进 `operations/version-matrix.md`**（条件分支如实写下：若付费系列改在客户端首发**之前**落地，该行整行不成立）；顺序纪律「矩阵先加、客户端后发」写明。
- **一处连带订正：** `contracts/_index.md` 的 purchase 域摘要仍写「后端只读、除表内**四项**外」，扩表后失真 ⇒ 单点改为「五项」，语义不动。
- **一处跨分片措辞消歧（orchestrator 裁）：** `purchase.md` 保证 8 原写「含该系列日后改走**免费轨道**……的情形」，而客户端本轮裁定**棘轮双向**（付费也永不改免费）⇒ 该情形在对侧已被禁止。保证 8 的语义不依赖它，删去该半句、只留「上架窗口关闭」一例，语义不变。
- **两个受保护小节原样未动**：`## 下一阶段` · `## derive 就绪度`。
