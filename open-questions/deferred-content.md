...
- **数值标杆的取值（08-02 定归宿 · 由焦点区移入）：** **卡牌产 / 削道念的量纲基准**（一张牌该产多少、一场内总产出相对 `baseMomentum` 的倍数、是否有道念相关的状态与倍率）与**回寿量三档的绝对点数**（标定口径 5% / 10% / 20% 已定，折算为 **50 / 100 / 200** 并与 `RarityTier` 一一绑定、`Charges = 1`、**三章通用不按篇章分条目**；**折算仍不构成定案**——三档点数仍欠取值，被实测改写时定价按 ×2 对齐关系自动跟随，改的是数不是形状。**⚠ 两份主题文档就本条相抵，待用户确认（09-12b 登记 · 本清单不裁决、不改主题文档）：** `systems/balance.md:1240` 仍写「折算不等于定案，三档点数仍欠取值」，而 `decisions/ADR-0248` 与 `systems/character-profile/life-span.md:50` 已把 **50 / 100 / 200、三章通用、不按篇章分条目**写死、且该文档 `## 待决问题` 现明写「无」。解读：**大概率是 `balance.md` 一句陈旧措辞未清**（`life-span.md` 与 ADR 是同一次定案的产物、彼此一致），但「究竟是已定案还是仍待实测校准」须由用户确认后才能从本清单移出。**「每章回寿事件次数」这一半已答结移出**，见 `../answer-logs/log-0911.md`），**留待内容扩充后的统计校准**——内容铺开且游戏可运行后由实测样本定出，归开发路线第 ③ 阶段；切入点是设计起始角色 starter deck 的过程。**并且优先打磨 ch1 内容。** **形状先于数值**：越级追分的量化形状已由 `systems/balance.md` 的形状锚点框住（一档层数差的产出差 ≈ 一档 `diff` 的 `baseMomentum` 落差，粒度为整副卡组），本条只欠取值、不欠形状。（**`lossPerMomentum` 的 ch2 / ch3 系数这一从属项已答结移出**，定为 5 / 10、三章全为定值，见 `../answer-logs/log-failure-and-punishment.md`。）→ `systems/balance.md`、`systems/character-profile/deck/`、`systems/character-profile/life-span.md`、`vision/scope.md`。
...
- **`narrative/` 分区的内部结构与文档清单（09-12 新增）：** 分区已建、承载什么已定（世界观设定 · 时间线 · 人物关系 · 碎片台账），**切成几份文档留给首次建区时按实际底稿量决定**。世界观底稿本身（各系列取哪个世界观实体、首批五角共享的是哪一场世界事件、五个独行者的故事线如何在暗处交叉）属内容阶段，是写系列名与角色背景的前置。→ `narrative/_index.md`、`content/character/_index.md`。
...
- **道念对比的视觉形态：** 主视觉地位已定；用什么形态（左右对比条 / 双数值 / 天平隐喻）、道念变化的反馈、「道念差」是否显式呈现均未定（后者的支持论据因 `lossPerMomentum` 而弱于逐点对应——各章需乘一个不同的篇章系数，三章定值 **10 / 5 / 10**）；**「还剩几回合」的呈现**（定长 10 回合的连带）亦未定。（寿元不常驻战斗屏、结算面板如实展示扣减量与扣后余量，已答结。）→ `ux/combat-ux.md`。
