...
- **成就条目目录：** 机制面整体已收口（schema · 采集面 · 两档族分配 60% 古宝 / 90% 法则 · 四条校验 · 组内启用成就数 ≥ 10）；欠的是**条目本身**——哪些成就、分几组、每组配哪两个专属的成就限定条目。依赖法则 / 古宝的条目先行。→ `content/achievement/`、`content/achievement-group/`、`systems/player-profile/achievement/`。
...
- **元进程持久化字段结构：** 仅剩**各账号级条目的解锁 / 获取 / 失去触发**待定。字段 schema 侧已全线收口：`AccountInfo`（仅余合规字段待后端）· `GameSetting` · 图鉴族 · `status`（启用 / 禁用）与「拥有 / 失去」两态的存档编码与写入通道 · **`Achievement` / `AchievementGroupState` 的条目形态与 `AchievementElements` / `AchievementTierElements` 两条写入通道**。→ `systems/services/profile-service.md`、`systems/player-profile/`。
- **PlayerPower 的获取 / 失去触发（09-07 收窄 · 公平性那半已答结移出）：** 方向已定为**轻度提升、PvE-only 可容忍**，且**道统残卷已给出一条获取渠道**（Finale 胜利时掷定并即时发放的概率掉落，规则已定案）；仍待定的是**具体在哪些 AdventureEvent 获取 / 失去**，以及开关 UI 的细化。（**「是否影响 cycle seed / 计分公平」与 pay/grind-to-win 的定性边界已答结**——残卷掷骰走账号级 RNG、不派生自 `CycleSeed`、不消耗任何子流 `State`，与轮回可复现性**两者不相交**；定性边界为「允许影响胜负、不应成为胜负的主要来源、不得随对局延长而累积、付费的战斗价值主要由古宝承载」，其两个百分比自标为评审参考、非承重。权威在 `systems/player-profile/player-power/_index.md` 的「意图」；移出记录见 `../answer-logs/log-0907.md`。）→ `systems/player-profile/player-power/`。
