...
- **jade（灵玉）的获取渠道与掉落权重（承重 · 本次归集 · 此前未作为独立条目进清单）：** 消耗侧已有形态（商店定价表「商品族 × 稀有度」），**产出侧一片空白**——哪些事件给灵玉、给多少、随篇章如何缩放均未定。**它卡住商店定价表的全部绝对数字**（产出侧空白时无从反推消耗侧），是 ch1 数值标杆专场里唯一一条「不先答就没法开工」的前置。→ `systems/character-profile/currency.md`、`systems/adventure-event/exchange/`、`systems/balance.md`。
...
- 以下主题文档仍是空占位或仅有骨架，尚无成形问题，待各自专场 handoff 播种：
  - 角色档案：`systems/character-profile/item/common-properties.md` 的共有字段无实质设计（`deck/` 已有骨架，但 `CardData` 的费用与触发器两格仍是结构占位）。
  - 玩家档案：`systems/player-profile/player-item/common-properties.md` 的共有字段无实质设计；`achievement/` 的条目 schema 与进度模型未设计。
