...
- **角色状态机。** `status = ongoing | defeated | completed`；`defeated` 为单一终态（`discarded` 降为其原因子类型），终态数据清理。
...
- **数据模型对齐。** PlayerProfile 持 `List<CharacterProfile>`；CharacterProfile 持 `status`、`chapter`、`Status`、`List<AdventureEvent>` 等（见 `20-systems/run-manager.md`）。
- **存档实现须原子 + 版本化**（见 ADR-0003 与 `state-save-rules.md`）；云端权威下存档为上行云端负载。
- **待办：** 重试上限若后续视作可调平衡项，归 `30-content/balance.md`；篇章衔接的「可用结束点」具体数据表达见 `20-systems/map-progression.md`。
