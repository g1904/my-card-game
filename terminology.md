...
| 图鉴（族） | Codex | **账号级的知识收集面**，共**六个**：`EnemyCodex` / `CharacterPowerCodex` / `PlayerPowerCodex` / `CharacterItemCodex` / `PlayerItemCodex` / `LocationCodex`。跨轮回持久、归 PlayerProfile、条目按内容 `Id` 索引、内容为**静态文案**（挂在对应 `Resource` 上），存档只记解锁状态。归 `systems/player-profile/codex/`。 |
