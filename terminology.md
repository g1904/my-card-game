...
| 状态置值 element | `StatusAssignment` | `ProfileChangeSpec.StatusChanges` 的条目：`StatusKey` + `IntValue` + `StringValue`（按 key 的声明类型取其一，另一格填缺省）。**语义是绝对置值**——赋一个已由组装方算好的值，`ProfileManager` 不做加减。承载 `CharacterProfile.Status` 上的规则字段（`CurrentLocationId` · `LocationEventCount` · 两个隐藏属性 band）。值类型与取值域逐行查封闭表 `StatusFields`；`Id` 型解析不到 → `PushError` + 整批拒绝。**恒不走 modifier pipeline。** |
...
| 放弃（角色） | `DefeatReason.Discarded` / 翻译键族 `MENU_DISCARD_*` | 玩家在主菜单「切换篇章」面上主动终结一个 `ongoing` 角色的动作，**终结三因之一**（规则层称「主动弃置」，见 `systems/services/life-cycle-service.md`）。**面向玩家的 UI 文案一律写「放弃」，不写「弃置」**——「弃置」在本库已被**功法侧**占用（见「功法」与「随售」两行），二者同词不同物：一个丢卡组里的一组牌，一个丢整个角色档且不可逆，而玩家在同一局里两处都会读到。**代码标识符与翻译键名不受此约束**（`Discarded` / `MENU_DISCARD_*` 照旧）——中文名与标识符字面不对应在本库已有先例（`Source.PackSell` ↔「随售」）。入口与确认形态归 `ux/screen-flow.md`。 |
...
| 道念 | momentum | **计分（scoring）用的胜利点数**，**`>= 0` 的 Integer**，双方各持一份：**战斗胜负 = 道念高者胜**。**由卡牌产出、可互相削减、下限为 0**（削减是饱和减法，**在每一次结算时就截断**，溢出的削减量不结转）；**战斗开始时双方各有一个由自身等级决定的起始道念 `baseMomentum`**。**标准 Combat = 10 个回合**（双方各 5 个）后比大小；**相等 = 平局，只发 `baseReward`、不扣寿元**。道念差在胜负两侧各驱动一件事：**胜 → 奖励厚度**（换算 = 两条支路：**强制奖励走线性 `1:1 × rewardPerMomentum` 单价**、**可选奖励走归一化 `advantage` 三档**，单价表见 `systems/balance.md`），**负 → 按差值 × `lossPerMomentum`（逐篇章系数，ch1 = 10）扣寿元**。道念是战斗内运行态，战斗结束即消失，不落 CharacterProfile。归 `systems/scoring.md`。 |
...
| 角色（模板） | CharacterData | **有身份的角色模板**（内容条目），区别于 `CharacterProfile`（某一次轮回的角色状态）。**开局由玩家从角色池中指定一个角色**（首批池规模 5，全池可选、无重抽通道、不做账号级解锁）；每个角色**自带一个神通与两门绑定功法 + 一个先天灵根**，且**每一局都相同**——角色因此是可辨认的身份，跨轮回的熟悉感由此产生。归 `systems/character-profile/`。 |
| 灵根 | `Affinity`（字段 `CharacterData.Affinities`） | **角色的先天资质**，五行五值封闭枚举（`Metal` / `Wood` / `Water` / `Fire` / `Earth`，外加 `Unspecified = 0` 哨兵）。它是**内容条目上的静态分类维度**（与 `Rarity` / `Pool` 同族），不产生任何运行时状态、不落存档。**唯一的规则后果是功法的硬性修习准入**；五行之间无相生相克，无契合度、无品级标签。首批五个角色各持一个单灵根。归 `systems/character-profile/`。 |
| 单灵根专属（功法） | `MaxCharacterAffinityCount == 1` | **只有单灵根角色能修的功法**。它是多灵根角色「可修池更宽 = 更强」的对冲手段：`MaxCharacterAffinityCount` 是功法对**角色灵根数量**的上限要求（`0` = 不限）。首批五个角色全为单灵根 ⇒ 该格暂不产生差异，但字段现在就在，以免加法窗口关闭后要给全部既有功法补格。与功法自身的 `MaxTier` 主语不同，不得混读。归 `systems/character-profile/deck/`。 |
| 功法 | CultivationTechnique | **卡组的构筑单位**：一组**必须整组入组**的卡牌（一张或多张），卡组由若干功法构成、数量不限。带**层数 `TechniqueTier`**；**层数提升 = 该组牌被整组替换为更强的一版**（每层一整套卡牌定义），不是数值变大、也不是追加新卡。**内容形态 = 轻量 header 条目持每层的卡牌 `Id` 列表**——每张卡各自是独立的内容条目，卡牌侧不带功法标记；一张卡可被多门功法引用。**功法不是卡组的完全划分**——业障与单卡奖励作为**游离散牌**照常进入卡组。**战斗内不感知功法**（组装时已展开为卡牌集合）；**存档存「功法 `Id` + 层数」而非展开后的牌**。每个角色**绑定两门功法**，其余靠 adventureEvent 升阶 / 弃置 / 学新，**弃置不设限（绑定的也能弃）**。**敌我共用同一 `CultivationTechniqueData` 类型与同一注册表**——敌方样本卡组同样由功法 + 层数展开（外加游离散牌）。带必填的 `Pool`（无默认，缺失 `PushError`；定义与两侧过滤语义见 `systems/character-profile/deck/_index.md` 的卡池划分一节），另带两格灵根相关字段 `RequiredAffinities` / `MaxCharacterAffinityCount`（修习准入，见同文档「灵根修习准入」一节）。归 `systems/character-profile/deck/`。 |
...
| 敌人图鉴 | EnemyCodex | 图鉴族之一（类 Pokédex）。只记录**静态知识**（这个敌人会做哪些事），**不记录动态情报**（它这回合做什么）——分工是「事前知识 vs 事中情报」。**敌人的行动不作事前预告，故本图鉴是事前知识的主通道。****遭遇即记录，不必击败**；**一次遭遇即全文案解锁**，词条含**四项** 人物背景 · 核心功法 · 运作方式 · 特点与弱点，其中 ③④ **写到招式与行为模式级**、词条页允许纵向滚动。**词条②「核心功法」= 功法 `Id` 非空列表**（多门、不含层数，每门须 ∈ 该敌人的功法列表）；**手写正文只写路数、不写功法名**，名字由引用字段在呈现层渲染，玩家可**点入功法图鉴读其逐层卡表**。 |
| 功法图鉴 | TechniqueCodex | 图鉴族之一：已接触过的功法（`CultivationTechnique`），条目按功法 `Id` 索引。**解锁有两条通道**：玩家自己习得（进入持有列表），以及**收录一个敌人时连带收录该敌人功法引用列表里的全部功法**。**词条 = 功法名 / 描述 / `Rarity` / 路数概括 / 逐层卡牌清单 + 可选 `CodexFlavor`**；卡表由呈现层从条目的成员卡现算、不落存档。玩家由敌人词条②的核心功法引用**反查出该敌人的样本卡组是设计目标**（兑现「同一个敌人第二次遇到就该能打赢」），界在「层数不进任何词条」。`Pool == Enemy` 的**敌方专用功法照常收录**，「敌方专属」是呈现层按 `Pool` 现算的标注、**不落存档**。标识符：`CodexKind.Technique` · 存档字段 `techniqueCodex`。归 `systems/player-profile/codex/`。 |
| 敌人模板 | EnemyData | 敌人的**静态内容数据**集合（稳定 `Id` + 图鉴文案 + 基准数值 + **功法引用列表 `TechniqueRef` + 游离散牌 `Id` 列表** + `KeyCardIds`（仅供 AI）+ 图鉴词条②的核心功法引用列表 `CoreTechniqueIds` + 可空的定制出牌策略 + `EncounterScopes` + `ChapterScope` + `PoolScope`；字段面见 `systems/enemies/`）。**样本卡组不再是直接的卡牌列表**，而是功法展开产物 ∪ 散牌，与玩家侧同构。**敌人等级不在模板上定死**：future-event-service 取一份模板 → 充实 / 改写 → 指派给该事件，等级是**物化产物**。与 `AdventureEventData ↔ EventOption`、`CardData ↔ CardInstance` 同属「模板 ↔ 实例」通则。**标识符统一为 `EnemyData`**（`XxxData` 是全库内容层的命名族），中文领域词保留「敌人模板」。归 `systems/enemies/`。 |
...
| 角色终结台词 | `DefeatLines`（元素 `DefeatLine`） | 挂 `CharacterData` 的一组第一人称台词，**三因各一句**（`Discarded` / `LifeSpanExhausted` / `FinaleFailed`）。**全作唯一一处可玩角色用自己的声音说话的地方**，只在 `CycleEndScreen` 上出现——普通战斗失败零台词，也不给任何其他场合。纯文本、无配音、逐角色定制；`LocalizedText` 类型、不落存档。与敌人侧的 `EnemyData.Lines`（战斗内五 slot 风味台词）是两套东西，互不影响。 |
| 灵石 | spiritStone | **轮回级基础货币**（软通货，官方货币名）：随轮回存在、随轮回清理（**跨篇章结转，与寿元同形**），归 CharacterProfile；获取 = **战斗产出**（灵石 ≈ 战利品），主要花销在 Exchange（交易 / 商店）。区别于每回合出牌资源 mana。与仙玉**不可兑换**。 |
| 仙玉 | immortalJade | **轮回级高阶货币**：同样随轮回存在、随轮回清理（**跨篇章结转，与寿元同形**），归 CharacterProfile。唯一**主动**获取通道 = **稀有 AdventureEvent 产出**（售出是同币回流，不新增渠道）；花销 = **高阶 Exchange 商品**。「高阶」由稀有度与价格量级表达，不由新机制表达；与灵石**不可兑换**。 |
...
| 收口 | `settle` | **非 MTG 词。** 战斗打满后的胜负判定 → 奖励计算 → 寿元扣减这一段 |
...
| 以物易物 | `ExchangeBarterRule` / `BarterOffer` / `Source.ExchangeBarter` | Exchange 支付侧的第二种形态：一个 offer 的支付侧不是一条货币 element，而是**一件由内容作者点名的轮回级法宝**。两种形态并存，一种不排除另一种。barter 由内容点名定值，**不经抽取池、不掷 `Shop` 子流、不参与刷新**；支付面仅法宝一族（账号级持有物不得作支付侧）。规则权威见 `systems/adventure-event/exchange/_index.md`，字段面与校验见其 `common-properties.md` |
| 持有查询 | `ProfileService.Holds(kind, scope, abilityId)` | 门面上的只读持有查询，未持有 = `false` 非错误。两个消费点：以物易物格的灰显、barter 提交路径的门面级前置拒绝。**它不是第二个 `CanAfford`**——`CanAfford` 只看资源 element，两者各管一半灰显判据，见 `systems/services/profile-service.md` |
