...
| 修行历程 | `pastEvent`（集合，`List<AdventureEvent>`） | 一个角色**已走过**的整段修行旅程——一条扁平的时序轨迹。**只有一种痕迹：进入并结算**（跳过通道已移除，见 `handoffs/2026-08-06c-...`）。**向前的走向不在此结构中**，由 future-event-service 每步现算的 eventOptions 决定。 | 同上 |
