...
| 敌人模板 | EnemyData | 敌人的**静态内容数据**集合（稳定 `Id` + 图鉴文案 + 基准数值 + **样本卡组** + `KeyCardIds` + `EncounterScopes` + `ChapterScope` + `PoolScope`）。**敌人等级不在模板上定死**：future-event-service 取一份模板 → 充实 / 改写 → 指派给该事件，等级是**物化产物**。与 `AdventureEventData ↔ EventOption`、`CardData ↔ CardInstance` 同属「模板 ↔ 实例」通则。**标识符统一为 `EnemyData`**（`XxxData` 是全库内容层的命名族），中文领域词保留「敌人模板」。归 `systems/enemies/`。 |
...
| 疲劳 | `fatigue` / `FatiguePerDraw` | 对位 MTG「从空牌库抽牌即输」与炉石的 fatigue，**本作两处都不同**：抽牌堆**不重洗**，空堆每抽一张扣 **1 点道念**（不递增、不致死，道念下限 0 逐次截断）；**它是规则不是关键字**，无载体、无卡面、不入栈，是抽牌流程内的一次直接扣减。**双方一视同仁**；扣减量是 `CombatRulesData` 上的全局常量，**不进 `EncounterSpec` 覆写组**（见 `systems/balance.md`） |
