...
已裁决）+ `draft-0812a.md` 末尾追加裁决 → ：**错误文案归属与版本提示片区一次收口** —— **① 玩家文案归 UI 层、键 = 后端 `code`、载体 = 翻译键**（`code → ERR_*` 是**机械变换**，不建第二张对照表；缺条目 → `PushWarning` + 按 `OpError` 回落四条通用文案 + 启动期审计；翻译资源**随包，不走 overlay**） · **② 三条「去更新」提示 = 同一根轴上的三档，同一时刻只呈现最高一档**（`UpgradeRequired` 复用常驻同步指示并**必须换掉「离线」二字**；软提示按 `recommendedVersion` 只提示一次），硬阻塞屏收敛为**一个屏 + 三变体表**，「去更新」地址 = **后端下发为主 + 随包 `ChannelConfig` 兜底 + 皆无则置灰** · **③ 迁移失败分两种情形**（超上界 → 走「需更新」变体；迁移抛错 → 「存档读取失败」变体 + 必上报），**否决重装与回退云端旧版**，绝不静默放行。同批新定：**`OpResult.Detail` 收口为诊断串**（作废 07-27b 的「面向玩家的原因串」）· **全库 UI 文案统一走翻译键**（中文默认、英文全占位符）。**⚠ 后端欠账：错误体 `detail` 增更新地址字段**　｜移出条数原记：3（另新定 2 条、新增待答 2 条）
