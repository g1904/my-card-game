...
**战斗流程收口** —— **① 先后手由 `EncounterSpec.FirstSide`（可空）决定**（future-event-service 物化写入，null → combat 子流掷） · **② 阵法与灵宠的区分轴 → 问题随灵宠删除而消失**（`CardType` 六值 → 五值，永久物统一归阵法，「实体 / 非实体」二分取消）。同批推翻 5 处既有定案：**抽牌堆不重洗、抽空即疲劳**（每张 −1 道念 ⇒ **道念削减通道从一条变两条**）· 起手 5 → **4** · 手牌上限 10 → **9** · 敌人卡组固定 15 → **两侧不设硬限** · 储物袋 99 → **9**（按 `Id` 堆叠计数）。新增**不设 mulligan**　｜移出条数原记：2（另新增 4 条待答）
