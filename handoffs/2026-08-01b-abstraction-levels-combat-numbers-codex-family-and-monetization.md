...
- date: 2026-08-01
- topic: systems/architecture · services/**（combat / future-event / life-cycle）· scoring · balance · game-progression · character-profile/life-total · player-profile/codex ⊃ 五图鉴 · player-profile/player-power · monetization（新增）· adventure-event/combat · ux/combat-ux · vision/scope · terminology
- status: distilled
- distilled-to: terminology.md, systems/character-profile/power/（新增）, systems/architecture.md, systems/services/（_index, combat-service, future-event-service, life-cycle-service）, systems/scoring.md, systems/balance.md, systems/game-progression.md, systems/monetization.md（新增）, systems/character-profile/life-total.md（由 life.md 改名）, systems/character-profile/_index.md, systems/adventure-event/combat/_index.md, systems/player-profile/codex/**（由 enemy-codex/ 重构并扩为图鉴族）, systems/player-profile/_index.md, systems/player-profile/player-power/_index.md, systems/_index.md, ux/combat-ux.md, vision/scope.md, decisions/ADR-0004, program-overview.md, system-overview.md, open-questions.md, answer-logs/log-0801b.md, .claude/rules/Context.md, `systems/character-profile/（life-total.md 由 life.md 改名并重写, _index.md, deck/, item/）`, `systems/adventure-event/combat/**`, `systems/services/（_index, combat-service, future-event-service, life-cycle-service, plot-manager）`, `systems/player-profile/（codex/** 由 enemy-codex/ 重构并扩族, _index.md, player-power/, player-item/）`, `systems/common-properties.md`, `.claude/（rules/Context.md, knowledge/architecture.md, knowledge/systems/_index.md, knowledge/autoloads/_index.md）`
...
- **`CharacterPower` = 轮回级的角色能力，对标账号级的 `PlayerPower`（已确认）。** 二者是同一个「能力」概念在两个生命周期层上的实例：**分界是生命周期，不是能力种类**（与 `PlayerItem` ↔ `CharacterItems` 同一条分界）。它沿用 PlayerPower 那套形状（`status` 开关、事件触发器被动修正、capability flag + modifier pipeline），由 CharacterProfile 持有、随轮回清理。新建 `systems/character-profile/power/`。
...
- **`CharacterPower` 与 PlayerPower 的复用边界。** 「对标」已定；仍待定二者是否共用同一份能力定义与 modifier pipeline、获取 / 失去触发、篇章突破是否随「全部继承」带入、与卡牌 / CharacterItems 的边界。→ `systems/character-profile/power/`。
- **其余四个图鉴的解锁触发。** EnemyCodex = 遭遇即记；PowerCodex / ItemCodex 的触发是「获得即记」「见过即记（含商店中见到）」还是「使用过即记」？→ `systems/player-profile/codex/`。
- **`EnemyTemplate` 与既有 `EnemyData` 是同一个东西吗？** 若是，则统一定名；若不是，则二者的分工需说明。且**物化后的敌人实例**尚未定名（`EnemyInstance`？随 `EventOption` 落存档还是战斗开始时再展开？）。→ `systems/services/future-event-service.md`、`systems/adventure-event/combat/`。
- **道念差 → 奖励厚度的换算。** 与「道念差 → lifeTotal 扣减」是同一条曲线的两侧，还是两套独立分档？上下限如何？→ `systems/balance.md`、`systems/adventure-event/combat/`。
- **卡牌产 / 削道念的量纲基准。** `baseMomentum` 给了起点，但「一张牌该产多少道念」「10 回合内一方总产出应达到起始值的几倍」这条基准未给——它决定越级追分是否可能。→ `systems/balance.md`、`systems/character-profile/deck/`。
- **Finale / Practice 是否也是 10 回合。** 二者是战斗变体、有独立胜负条件；固定回合数是否照搬未陈述。→ `systems/adventure-event/finale/`、`practice/`。
- **`lifeTotal` 的回复幅度与基线成长。** 「通过 event 恢复」已定；单次回复幅度、更高境界的 `lifeTotalLimit` 基线、以及 `lifeTotalLimit` 是否也由事件推拉（与 `manaLimit` 同模型）仍未定。→ `systems/balance.md`。
- **premium bundle 的商业化细则。** 一次性还是可重复购买？定价与地区？是否有其他付费点（外观 / 通行证）？随机 PlayerPower 是否与「道统残卷」概率互相影响？付费改写重试上限如何在 UX 上呈现（避免「付费才玩得下去」的观感）？→ `systems/monetization.md`。
- **重试上限可变后的存档表达。** 「无限 / 9 / 3」由账号级礼包状态改写，故重试上限的判定要读 PlayerProfile 的持有状态——它是一个 `capability flag`、一个 `modifier pipeline` 的具名修正，还是一个独立的 `Entitlement` 字段？→ `systems/player-profile/`、`systems/services/profile-service.md`。
- **第四 / 第五级（processor / handler）目前无实例。** 名字先定下，但何时该下沉到第四级、判据是什么未给——与 service / manager 各有三判据不同，module 以下暂无判据。→ `systems/architecture.md`。

## Notes / triage

- 本 handoff 是 `inbox/draft-0801b.md` 的整理稿，是 08-01 道念改写的**直接续篇**：把那次改写留下的「终止条件 / 产出途径 / 分界值 / 等级来源 / life 归 0」五个承重缺口一次性填上。
...
- **结构改动：** `systems/player-profile/enemy-codex/` 重构为 `systems/player-profile/codex/`（族目录 + `enemy-codex.md`）；`systems/character-profile/life.md` 改名为 `life-total.md`；新增 `systems/character-profile/power/`（CharacterPower）与 `systems/monetization.md`。
