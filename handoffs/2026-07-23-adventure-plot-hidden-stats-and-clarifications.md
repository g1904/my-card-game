...
- status: distilled
- distilled-to: terminology.md, vision/scope.md, vision/pillars.md, vision/references.md, ux/screen-flow.md, ux/onboarding.md, systems/run-manager.md, systems/adventure-event-combat.md, systems/energy-economy.md, systems/map-progression.md, systems/adventure-plot.md, 30-content/events.md, 30-content/balance.md, 30-content/adventure-events.md, decisions/ADR-0002-adventure-event-taxonomy.md, decisions/ADR-0003-online-cloud-authority.md, decisions/ADR-0004-realm-checkpoint-retry-model.md
...
- **新抽象 AdventurePlot（修行剧情 / 隐藏剧情线）：** 一棵由不同分支可能性构成的**树**，**调制 future-event-service 产出的 `eventOptions`**（下一批可选的修行事件）。它像一条在背景中运行的**隐藏故事线**；也可像 **DnD** 那样，在某些节点**让玩家选择要走哪条分支**。
- **隐藏属性驱动剧情线（示例）：**
  - **煞气点数（Bloodlust）** 作为隐藏属性——积累到一定阈值会触发**「煞气反噬」**故事线。
...
- **AdventurePlot 的编码与授权方式（新，未定）。** 「一棵分支可能性的树，调制 eventOptions」如何用数据表达？它挂在 future-event-service 的产出链路何处——是**改写 / 加权**候选池的上层调制器，还是另一套并行结构？DnD 式「让玩家选分支」在何处、以何 UI 触发？
...
- **寿元 / lifeSpan 与 life 的边界（新，需确认）。** 明确 寿元 是独立于血量 life 的隐藏数值；但它是否有上限、如何随轮回推进增长、「大限将至」触发后果（角色 defeated？转入 Finale？）未定。
...
- **PlayerPower 获取 / 失去触发与公平性（沿用，未闭合）。** 方向已定为「轻度、PvE-only 可容忍」；但具体在哪些 AdventureEvent 获取 / 失去、是否影响 cycle seed / 计分公平仍待定。
- **后端 / 账号系统选型与合规（沿用，未定）。** 见第 3 条；PIPL / 实名 / 渠道审核 / 账号注销 / 数据导出。删除游客态后**强制实名 / 登录**进一步抬高合规门槛。

## Notes / triage

承接式批量裁定 + 一个新抽象。路由：
- 断线缓冲、删除游客态 → `vision/scope.md`（平台约束 · 登录渠道去游客）、`ux/screen-flow.md`（登录入口去 Guest）、`ux/onboarding.md`、`systems/run-manager.md`（账号 / 同步维度）。
- PlayerPower 轻度提升 → `systems/run-manager.md`、`ux/screen-flow.md`。
- AdventurePlot + 隐藏属性（煞气 / 寿元）→ `30-content/events.md`（隐藏剧情线 / 分支）、`systems/run-manager.md`（隐藏属性归入 `Status`）、`terminology.md`（新术语）。
- 成就 60% / 90% 双档一次性、80/20 可见 → `ux/screen-flow.md`。
- Combat 子类型命名、Finale 独立类型 → `systems/adventure-event-combat.md`、`terminology.md`；**ADR 候选**（Finale 是否入六类枚举 → 需修订 ADR-0002）。
- 横向滑动选择区 → `systems/map-progression.md`（选择 UI）。
- 无 mana 曲线 · 上限 + 逐步恢复 · 炼气基线 10/10 · 5/5 → `systems/energy-economy.md`、`systems/adventure-event-combat.md`、`30-content/balance.md`。
...
3. **AdventurePlot 是剧本体系，四级结构（定案 + 充实）。** **Story**（贯穿三大篇章的主线）> **Chapter**（单篇章剧本，三个相连成 Story）> 篇章内穿插 **SideChapter** / 跨篇章穿插 **SideStory**。**Character 只记录 key points**；**完整剧本与分支内容存于云端剧本服务（script service）**，按 key points 请求。→ **新建 `systems/adventure-plot.md`**；`terminology.md`、`events.md`、`run-manager.md`、`map-progression.md`。
4. **后端 / 账号 = 重账号（定案，参考三国杀 Online）。** 为支撑云端同步存档采用**重账号**路线。→ `vision/scope.md`、`ADR-0003`。
