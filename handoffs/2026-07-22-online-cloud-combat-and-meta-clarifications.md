...
- status: distilled
- distilled-to: vision/scope.md, vision/references.md, systems/run-manager.md, systems/map-progression.md, systems/adventure-event-combat.md, systems/energy-economy.md, ux/screen-flow.md, ux/onboarding.md, terminology.md
...
- 影响：`vision/scope.md` 原「离线可玩 + 云同步混合模型（本地 `user://` 权威）」被取代；MVP 的「离线可玩」被取代。这也与项目根约定 `.claude/CLAUDE.md`「玩法完全离线」直接冲突（见 Open questions，需确认是否连同工具约定一并调整）。
...
承接式批量裁定。路由：
- 存档模型反转（在线 / 云端权威）→ `vision/scope.md`（取代混合模型段与 MVP 离线条）+ `systems/run-manager.md`（同步冲突 = 云端权威）。**ADR 候选**：以「强制在线 · 云端权威」取代先前「混合存档」ADR 候选。
- 战斗模型 = life + mana（MTG / Hearthstone）→ `systems/adventure-event-combat.md` + `systems/energy-economy.md`（mana 资源）+ `vision/references.md`（新增两条参照）。
- 节点形态 = 月圆之夜风格 → `systems/map-progression.md`（节点 map 形态）+ `references.md`（补「借鉴」）。
- 篇章继承（全部）、状态分类法（defeated 单终态）、ongoing 语义、重试上限、篇章解锁、PlayerPower/PlayerItem/Achievements 语义 → `systems/run-manager.md`、`systems/map-progression.md`、`ux/onboarding.md`、`ux/screen-flow.md`。
- 成就加权发放、视频 = VideoStreamPlayer、平台优先级 → `ux/screen-flow.md`、`vision/scope.md`。
