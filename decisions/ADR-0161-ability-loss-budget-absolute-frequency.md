...
- **合计取 ≈0.83** — 否决：**总额是承重表述，压低它会同批压低全部持久支**，而这三支各自的量级是按反相关轴分配出来的、不该被一个总额取值统一稀释。
  > **不采用「每 7 轮回一次 ⇒ 几乎不可见」这条理由。** 本作是 roguelike，玩家会反复开轮回——**小概率事件在深度游玩下必然发生**，每 7 轮回一次的机制照样会被遇到、被记住。可见性不由单轮回概率判定，故它不是一条能用来否决取值的判据。
