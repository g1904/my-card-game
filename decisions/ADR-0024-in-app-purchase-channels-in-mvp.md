...
- **三渠道对应既定的平台优先级**（移动端优先 → 微信 / QQ 其次 → 海外与跨平台最后，见 `vision/scope.md`），少任何一个都会让某一主要平台上 MVP 唯一的付费点不可用。
...
- **把支付接入推迟到 MVP 之后** — 否决：兑现段与 `PlayerEntitlement` 已在 MVP 内成型，购买段缺席会让 MVP 唯一的付费点端到端不可用，且日后补入时要重做 Store 屏与导出配置。
