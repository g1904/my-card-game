...
- **角色状态机。** `status = ongoing | defeated | completed`；`defeated` 为单一终态，**原因恰三种**：主动弃置 `discarded`、**寿元 / lifeSpan 归 0（大限将至）**、**渡劫失败 `FinaleFailed`**。前两种是资源触底，由资源表驱动判定；末一种是篇章闸门，走终态判定上的一条显式旁路。终态数据清理。
