...
- **角色状态机。** `status = ongoing | defeated | completed`；`defeated` 为单一终态，**原因恰三种**：主动弃置 `discarded`、**寿元 / lifeSpan 归 0（大限将至）**、**渡劫失败 `FinaleFailed`**。前两种是资源触底，由资源表驱动判定；末一种是篇章闸门，走终态判定上的一条显式旁路。**`defeated` 的终态数据清理只清运行态与可回滚实体状态，保留身份与元进程格；`completed` 的角色实体状态不被清理**（保留即境界存档），见 `systems/services/life-cycle-service.md`。
  - **`Practice` / `Standard` 档输掉一场战斗本身不终结角色**（只按道念差扣寿元，见 `systems/scoring.md`）——这两档的失败不是独立的终结原因；扣到 0 时走的仍是 `LifeSpanExhausted`。
  - **`Finale` 档失败即终结**：篇章边界的胜负就是篇章推进的闸门，败则该角色当场终结、本篇章不推进（见 `systems/adventure-event/combat/_index.md`）。那笔按道念差扣的寿元照常扣，只是不再是死亡判据。
