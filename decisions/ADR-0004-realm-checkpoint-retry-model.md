...
- **角色状态机。** `status = ongoing | defeated | completed`；`defeated` 为单一终态，**原因收敛为三种**：主动弃置 `discarded`、**寿元 / lifeSpan 归 0（大限将至）**、**`lifeTotal` 归 0**。**输掉一场战斗本身不终结角色**（只按道念差扣 `lifeTotal`，见 `systems/scoring.md`），故原先的「战斗失败」不再是独立的终结原因。终态数据清理。
