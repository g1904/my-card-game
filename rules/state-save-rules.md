...
- **确定性的边界 = 同一 `contentVersion` 内。** 给定种子在**同一内容版本下**复现同一个轮回；但内容热更**以 overlay 更新为准**（轮回进行中更新即生效，不冻结 `contentVersion`），因此**不承诺跨内容版本的可复现性**——线上随时修正数值的价值高于跨版本复现。方向来源：`game-design-documents/10-handoffs/2026-07-26-event-priority-skip-semantics-and-hotfix-scope.md`。
